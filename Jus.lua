...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    hd.InputEnded:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
            dragStart = nil
            -- Koreksi Y sidebar setelah drag selesai
            task.defer(syncSidebarY)
        end
    end)
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dragStart and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            ...
            sidebar.Position = UDim2.new(0, sbXFromMAbsX(mAbsX), 0, sidebar.Position.Y.Offset)
        end
    end)
    ...
    workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(function()
        task.spawn(function()
            local vp = workspace.CurrentCamera.ViewportSize
            m.Position = UDim2.new(0, vp.X/2 - w/2, 0, vp.Y/2 - h/2)
            task.wait(0.1)
            m.Position = UDim2.new(0, vp.X/2 - w/2, 0, vp.Y/2 - h/2)
            syncSidebarY()
        end)
    ...
    -- ============================================
    -- HELPER: hitung posisi X sidebar dari posisi X absolut m
    -- ============================================
    local function sbXFromMAbsX(mAbsX)
        return mAbsX - 48 - 6
    end

    -- HELPER: set sidebar Y dan Size agar sejajar m (panggil setelah m settle)
    local function syncSidebarY()
        local mPos  = m.AbsolutePosition
        local mSize = m.AbsoluteSize
        sidebar.Position = UDim2.new(0, sidebar.Position.X.Offset, 0, mPos.Y + 10)
        sidebar.Size     = UDim2.new(0, 48, 0, mSize.Y - 20)
    end

    -- ============================================
    -- FUNGSI SHOW / HIDE ANIMASI
    -- ============================================
    ...
    local function showHub()
        ...
        -- Posisi final m dalam piksel
        local mFinalAbsX = vp.X / 2 - w / 2
        local mFinalAbsY = vp.Y / 2 - h / 2

        -- Set posisi awal sidebar (di luar kanan, Y sudah benar)
        sidebar.Size     = UDim2.new(0, 48, 0, h - 20)
        sidebar.Position = UDim2.new(0, sbXFromMAbsX(offX), 0, mFinalAbsY + 10)
        sidebar.Visible  = true
        m.Position       = UDim2.new(0, offX, 0, mFinalAbsY)
        m.Visible        = true

        -- Tween keduanya masuk bareng
        T:Create(m, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, mFinalAbsX, 0, mFinalAbsY)
        }):Play()
        T:Create(sidebar, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, sbXFromMAbsX(mFinalAbsX), 0, mFinalAbsY + 10)
        }):Play()

        task.delay(0.5, function()
            hubVisible   = true
            hubAnimating = false
            syncSidebarY()  -- koreksi Y setelah settle
        end)
    end
    ...
    local function hideHub()
        ...
        -- Tween m dan sidebar keluar bareng ke kanan
        T:Create(m, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Position = UDim2.new(0, offX, 0, m.AbsolutePosition.Y)
        }):Play()
        T:Create(sidebar, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
            Position = UDim2.new(0, sbXFromMAbsX(offX), 0, sidebar.Position.Y.Offset)
        }):Play()
    ...
    local offX0      = vp0.X + 50
    local mFinalAbsX = vp0.X / 2 - w / 2
    local mFinalAbsY = vp0.Y / 2 - h / 2

    sidebar.Size     = UDim2.new(0, 48, 0, h - 20)
    sidebar.Position = UDim2.new(0, sbXFromMAbsX(offX0), 0, mFinalAbsY + 10)
    sidebar.Visible  = true
    m.Position       = UDim2.new(0, offX0, 0, mFinalAbsY)
    m.Visible        = true
    ...
    task.delay(0.55, function() syncSidebarY() end)
