...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    local toggleSidebar = sb.toggleSidebar
    ...
    local function showConfirmDialog()
        ...
        if mn then
            ...
            cb.Text = "−"
            task.delay(0.25, function() sb.showPill() end)
            task.wait(0.6)
        end
        ...
        local pillOverlay = Instance.new("Frame")
        pillOverlay.Name                   = "PillOverlay"
        pillOverlay.Size                   = UDim2.new(1, 0, 1, 0)
        pillOverlay.BackgroundColor3       = Color3.fromRGB(0, 0, 0)
        pillOverlay.BackgroundTransparency = 1
        pillOverlay.BorderSizePixel        = 0
        pillOverlay.ZIndex                 = 2000
        pillOverlay.Parent                 = pill
        Instance.new("UICorner", pillOverlay).CornerRadius = UDim.new(0.5, 0)
        local pB = Instance.new("TextButton", pillOverlay)
        pB.Size = UDim2.new(1,0,1,0); pB.BackgroundTransparency=1; pB.Text=""; pB.ZIndex=2001; pB.Active=true; pB.AutoButtonColor=false
        ...
        T:Create(pillOverlay,   TweenInfo.new(0.25), {BackgroundTransparency = 0.5}):Play()
        ...
        local function closeDialog()
            ...
            T:Create(pillOverlay,   TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
            task.wait(0.2)
            blur:Destroy()
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        mn = not mn
        if mn then
            if sb.getSidebarOpen() then toggleSidebar() end
            sb.hidePill()
        else
            task.delay(0.25, function() sb.showPill() end)
        end
    ...
    local function applyTheme(theme)
        ...
        applyToElement(m)
        applyToElement(sidebar)
        pill.BackgroundColor3   = colors.bg
        sidebar.BackgroundColor3 = colors.bg
        sb.pillStroke.Color     = colors.br
        sb.pillLabel.TextColor3 = colors.tx
    ...
    if themesModule and themesModule.List and #themesModule.List > 0 then
        -- Grid container
        -- Card container pembungkus grid
        local gridCard = Instance.new("Frame")
        gridCard.Size             = UDim2.new(1, 0, 0, 0)
        gridCard.AutomaticSize    = Enum.AutomaticSize.Y
        gridCard.BackgroundColor3 = Co.cd
        gridCard.BorderSizePixel  = 0
        gridCard.LayoutOrder      = nO()
        gridCard:SetAttribute("ThemeTag", "cd")
        gridCard.Parent           = currentSc
        Instance.new("UICorner", gridCard).CornerRadius = UDim.new(0, 12)
        local gridCardStroke = Instance.new("UIStroke", gridCard)
        gridCardStroke.Color     = Co.br
        gridCardStroke.Thickness = 1.5
        local gridCardPad = Instance.new("UIPadding", gridCard)
        gridCardPad.PaddingLeft   = UDim.new(0, 12)
        gridCardPad.PaddingRight  = UDim.new(0, 12)
        gridCardPad.PaddingTop    = UDim.new(0, 12)
        gridCardPad.PaddingBottom = UDim.new(0, 12)
        ...
        for i, theme in ipairs(themesModule.List) do
            ...
            cell.Size             = UDim2.new(0, 52, 0, 52)
            cell.BackgroundColor3 = ac
            cell.Text             = ""
            ...
            cell.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or
                   input.UserInputType == Enum.UserInputType.Touch then
                    T:Create(cellScale, TweenInfo.new(0.08, Enum.EasingStyle.Quad), { Scale = 0.88 }):Play()
                end
            end)
            cell.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or
                   input.UserInputType == Enum.UserInputType.Touch then
                    T:Create(cellScale, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Scale = 1 }):Play()
                end
            end)
            ...
            cell.MouseEnter:Connect(function()
                if theme.ID ~= config.CurrentTheme then
                    T:Create(cell, TweenInfo.new(0.15), {BackgroundColor3 = theme.Colors and theme.Colors.ach or ac}):Play()
                end
            end)
            cell.MouseLeave:Connect(function()
                T:Create(cell, TweenInfo.new(0.15), {BackgroundColor3 = ac}):Play()
            end)
        end
    else
    ...
    hd.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
            dragStart = i.Position; startPos = m.Position
        end
    end)
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dragStart and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            ...
            m.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + dt.X, startPos.Y.Scale, startPos.Y.Offset + dt.Y)
            task.defer(function()
                if not m or not m.Parent then return end
                local mX = m.AbsolutePosition.X
                local mY = m.AbsolutePosition.Y
                local mH = m.AbsoluteSize.Y
                local pillY = mY + (mH/2) - (72/2)
                if sb.getSidebarOpen() then
                    local sbX = mX - 48 - 6
                    sidebar.Position = UDim2.new(0, sbX, 0, mY + 10)
                    pill.Position    = UDim2.new(0, sbX - 22 + (-4), 0, pillY)
                else
                    pill.Position = UDim2.new(0, mX - 22 + (-4), 0, pillY)
                end
            end)
        end
    end)
        ...
        mT.InputEnded:Connect(function(input)
            if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
                ...
                if not wasDragged then
                    ...
                    if m.Visible then playSound(Sounds.Click,0.5); btnText.Text="Hide"; mI.Image="rbxassetid://114167695335193"
                    else playSound(Sounds.Click,0.4); btnText.Text="Show"; mI.Image="rbxassetid://99334701468696" end
    ...
    -- Pill muncul setelah animasi masuk selesai
    task.delay(0.6, function() sb.showPill() end)
