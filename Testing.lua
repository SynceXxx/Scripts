...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    -- "OPEN" label (rotated, shown when collapsed)
    local openLabel = Instance.new("TextLabel")
    openLabel.Name             = "OpenLabel"
    openLabel.Size             = UDim2.new(0, 80, 0, SB_CLOSED_W - 4)
    openLabel.AnchorPoint      = Vector2.new(0.5, 0.5)
    openLabel.Position         = UDim2.new(0.5, 0, 0.62, 0)
    ...
    -- Full-sidebar clickable area (ZIndex 3, behind tab buttons at 4)
    ...
    m:GetPropertyChangedSignal("AbsolutePosition"):Connect(syncSidebarPos)
    m:GetPropertyChangedSignal("AbsoluteSize"):Connect(syncSidebarPos)
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dS and (i.UserInputType == Enum.UserInputType.MouseMovement or
            ...
            -- syncSidebarPos is called automatically via AbsolutePosition signal
        end
    end)
