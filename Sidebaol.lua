...
function Sidebar.create(g, m, Co, onTabSwitch)
    ...
    m:GetPropertyChangedSignal("Visible"):Connect(function()
        if not m.Visible then
            -- Sembunyikan pill & sidebar langsung saat UI di-hide
            pill.Visible    = false
            sidebar.Visible = false
        else
            if not pillShouldShow then return end
            task.spawn(function()
                -- Tunggu 3 frame biar AbsolutePosition benar-benar settle
                task.wait()
                task.wait()
                task.wait()
                ...
                -- Kalau sidebar open, animasikan sidebar masuk bareng pill
                -- (bukan langsung visible, supaya sync dengan main frame)
                if sidebarOpen then
                    local sbX = mX - SB_W - SB_GAP
                    local sbY = mY + SB_Y_OFFSET
                    local sbH = mH - SB_Y_OFFSET * 2

                    -- Mulai dari posisi tersembunyi (di belakang main frame)
                    sidebar.Position = UDim2.new(0, mX - PILL_W, 0, sbY)
                    sidebar.Size     = UDim2.new(0, SB_W, 0, sbH)
                    sidebar.Visible  = true

                    -- Animasi slide keluar bareng
                    T:Create(sidebar, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                        Position = UDim2.new(0, sbX, 0, sbY)
                    }):Play()
                end

                -- Pill muncul setelah animasi sidebar selesai
                task.wait(0.35)
                showPill()
            end)
        end
    ...
    local function showPill()
        ...
        -- Tunggu 1 frame biar AbsolutePosition settle
        task.wait()
        ...
        -- Hitung posisi final pill
        local pillX
        if sidebarOpen and sidebar.Visible then
            local sbX = mX - SB_W - SB_GAP
            pillX = sbX - PILL_W + PILL_OVERLAP
        else
            pillX = mX - PILL_W + PILL_OVERLAP
        end
        local finalPos = UDim2.new(0, pillX, 0, pillY)

        -- Set label sesuai state
        pillLabel.Text             = sidebarOpen and "CLOSE" or "OPEN"
        pillLabel.TextTransparency = 0

        -- Start dari dalam tepi kanan main frame, slide ke kiri
