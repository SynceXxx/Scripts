...
local function showConfirmDialog()
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        cb.Text = mn and "+" or "−"
        -- Reset sidebar ke posisi tertutup saat minimize
        if mn then
            sidebarOpen = false
    ...
    -- Strategi: sidebar SELALU visible, posisi X yang berubah
    -- Saat tertutup: geser ke kiri sampai hanya toggleBtn yang keliatan
    -- Saat terbuka: geser ke kiri penuh
    -- toggleBtn child of sidebar, selalu nempel di kanannya
    -- ============================================
    local sidebarW    = 52
    local toggleW     = 18
    local GAP         = 6
    ...
    -- Posisi saat TERTUTUP: sidebar hampir semua tersembunyi di kiri hub
    -- hanya toggleBtn (di sisi kanan sidebar) yang keliatan
    -- toggleBtn.Position.X = 1 (sisi kanan sidebar) + 2px gap
    -- Jadi lebar yang keliatan = toggleW + 2
    local closedX = -(sidebarW - toggleW/2 + 2)  -- sebagian sidebar keliatan = cuma tombol
    local openX   = -(sidebarW + GAP)             -- sidebar penuh keluar

    -- Sidebar frame — child of m, selalu visible
    ...
    sidebar.ZIndex = 1
    sidebar.Visible = true  -- selalu visible!
    ...
    -- Toggle button — child of sidebar, nempel di sisi kanan
    -- Selalu ikut slide bareng sidebar
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, toggleW, 0, 60)
    toggleBtn.AnchorPoint = Vector2.new(0, 0.5)
    toggleBtn.Position = UDim2.new(1, 2, 0.5, 0)  -- sisi kanan sidebar + 2px gap
    ...
    -- Buka/tutup sidebar dengan slide
        ...
        mT.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if not wasDragged then
                    ...
                    -- Sidebar ikut hide/show — reset ke posisi tertutup
