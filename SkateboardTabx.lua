...
local function showConfirmDialog()
    ...
    -- Input Box creation function
    local function In(n, cK, callback)
        local ca = Instance.new("Frame")
        ca.Size = UDim2.new(1, 0, 0, bH)
        ca.BackgroundColor3 = Co.cd
        ca.LayoutOrder = nO()
        ...
        local iB = Instance.new("TextBox")
        iB.Size = UDim2.new(0, 100, 0, 32)
        iB.Position = UDim2.new(1, -114, 0.5, -16)
        iB.BackgroundColor3 = Co.ch
        iB.Text = tostring(C[cK] or "")
        iB.TextColor3 = Co.tx
        ...
        local st = Instance.new("UIStroke", iB)
        st.Color = Co.br
        st.Thickness = 1
        
        iB.Focused:Connect(function()
            T:Create(st, TweenInfo.new(0.2), {Color = Co.ac, Thickness = 2}):Play()
            T:Create(iB, TweenInfo.new(0.2), {BackgroundColor3 = Co.al}):Play()
        end)
        
        iB.FocusLost:Connect(function()
            T:Create(st, TweenInfo.new(0.2), {Color = Co.br, Thickness = 1}):Play()
            T:Create(iB, TweenInfo.new(0.2), {BackgroundColor3 = Co.ch}):Play()
        ...
        iB.MouseEnter:Connect(function()
            if not iB:IsFocused() then
                T:Create(iB, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(45, 45, 50)}):Play()
            end
        end)
        
        iB.MouseLeave:Connect(function()
            if not iB:IsFocused() then
                T:Create(iB, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
            end
        end)
    ...
    -- Build UI content for Skateboard Training
    Se("Lucky Wheel")
    Tg("Auto Ticket Lucky Wheel", "AutoTicket", function(enabled)
        Feature.toggleAutoTicket(enabled)
    end, "rbxassetid://121640601863855")
    
    Sl("Ticket Delay", 0.1, 2, "TicketDelay", function(val)
        Feature.updateTicketDelay(val)
    end)
    
    Se("Training")
    Bt("OP Auto Train", function()
        Feature.doAutoTrain()
    end, "rbxassetid://140507950554297")
    
    Se("Gacha Spin")
    In("Spin Amount", "SpinAmount", function(val)
        Feature.updateSpinAmount(val)
    end)
    
    Sl("Spin Delay", 0.1, 3, "SpinDelay", function(val)
        Feature.updateSpinDelay(val)
    end)
    
    Tg("Auto Spin", "AutoSpin", function(enabled)
        Feature.toggleAutoSpin(enabled)
    end, "rbxassetid://93830334871541")
    
    Bt("Manual Spin", function()
        Feature.doManualSpin()
    end, "rbxassetid://85752972683600")
