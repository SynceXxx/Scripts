    ...
    local function applyTheme(theme)
        ...
        if hd then
            ...
            for _, child in ipairs(hd:GetChildren()) do
                ...
                elseif child:IsA("TextButton") then
                    -- Buttons in header
                    pcall(function()
                        T:Create(child, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                        
                        -- Update icon color if button has ImageLabel child
                        local icon = child:FindFirstChildOfClass("ImageLabel")
                        if icon then
                            T:Create(icon, tweenInfo, {ImageColor3 = colors.ts}):Play()
                        end
                    end)
                end
            end
        ...
        if m then
            for _, child in ipairs(m:GetChildren()) do
                ...
                elseif child:IsA("ScrollingFrame") then -- Scroll container
                    pcall(function() 
                        T:Create(child, tweenInfo, {ScrollBarImageColor3 = colors.br}):Play()
                    end)
                end
            end
        end
        
        -- Update color scheme for future elements
        Co.bg = colors.bg
        Co.cd = colors.cd
        Co.ch = colors.ch
        Co.br = colors.br
        ...
        local function updateElement(element)
            ...
            pcall(function()
                if element:IsA("Frame") or element:IsA("ScrollingFrame") then
                    -- Update ANY frame that uses old theme colors
                    local bgColor = element.BackgroundColor3
                    
                    -- Check if it's using old bg color
                    if bgColor == Co.bg then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.bg}):Play()
                    -- Check if it's using old card/section color
                    elseif bgColor == Co.cd then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                    -- Check if it's using old input/hover color
                    elseif bgColor == Co.ch then
                    ...
                    if element:IsA("ScrollingFrame") then
                        T:Create(element, tweenInfo, {ScrollBarImageColor3 = colors.br}):Play()
                    end
                    
                elseif element:IsA("TextLabel") then
                    -- Update text colors based on current color
                    local textColor = element.TextColor3
                    
                    if textColor == Co.tx then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    elseif textColor == Co.ts then
                        T:Create(element, tweenInfo, {TextColor3 = colors.ts}):Play()
                    end
                    
                elseif element:IsA("TextButton") then
                    -- Skip header buttons (close, minimize, theme)
                    if not (element.Text == "×" or element.Text == "−" or element:FindFirstChildOfClass("ImageLabel")) then
                        
                        local bgColor = element.BackgroundColor3
                        
                        -- Check if this is a toggle switch (has Frame child and no text)
                        local hasFrameChild = element:FindFirstChildOfClass("Frame") ~= nil
                        local isToggle = element.Text == "" and hasFrameChild
                        
                        if isToggle then
                            -- Toggle switch - detect ON/OFF by color
                            -- ON = blue-ish color (R < 0.3, G > 0.3, B > 0.6)
                            local isOn = (bgColor.R < 0.3 and bgColor.G > 0.3 and bgColor.B > 0.6)
                            
                            if isOn then
                                -- Update to new accent color
                                T:Create(element, tweenInfo, {BackgroundColor3 = colors.ac}):Play()
                            else
                                -- Update to new ch color (gray/dark)
                                T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                            end
                        else
                            -- Regular button - update background and text
                            if bgColor == Co.ch then
                                T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                            elseif bgColor == Co.cd then
                                T:Create(element, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                            elseif bgColor == Co.ac then
                                T:Create(element, tweenInfo, {BackgroundColor3 = colors.ac}):Play()
                            end
                            
                            -- Update text color
                            if element.TextColor3 == Co.tx then
                                T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                            elseif element.TextColor3 == Co.ts then
                                T:Create(element, tweenInfo, {TextColor3 = colors.ts}):Play()
                            end
                        end
                    end
                ...
                elseif element:IsA("TextBox") then
                    -- Update textbox backgrounds and text
                    if element.BackgroundColor3 == Co.ch then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                    end
                    if element.TextColor3 == Co.tx then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    end
                    if element.PlaceholderColor3 == Co.ts then
                        T:Create(element, tweenInfo, {PlaceholderColor3 = colors.ts}):Play()
                    end
                    
                elseif element:IsA("ImageLabel") or element:IsA("ImageButton") then
                    -- Update image colors (for icons)
                    if element.ImageColor3 == Co.ts then
                        T:Create(element, tweenInfo, {ImageColor3 = colors.ts}):Play()
                    elseif element.ImageColor3 == Co.tx then
                        T:Create(element, tweenInfo, {ImageColor3 = colors.tx}):Play()
                    end
                end
            end)
        ...
        -- Show notification
        Feature.showNotification("Theme: " .. theme.Name, false)
