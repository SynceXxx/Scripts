...
local function showConfirmDialog()
    ...
    local function Ib(n, cK)
        ...
        local initialValue = tonumber(C[cK])
        if initialValue and initialValue > 0 then
            ib.Text = tostring(initialValue)
        end
    ...
    -- Play/Stop Toggle
    local playToggle = Tg("Play Performance", "IsPlaying", function(enabled)
        local success = Feature.togglePlayback(enabled)
    ...
    -- Saved Songs List (Display only, managed in features)
    -- TODO: Add list display or use notifications
