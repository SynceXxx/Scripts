...
local function showConfirmDialog()
    ...
    -- Input Box creation function (dari CoasterTab - dengan validasi)
    local function In(n, cK)
        local ca = Instance.new("Frame")
        ca.Size = UDim2.new(1, 0, 0, bH)
        ca.BackgroundColor3 = Co.cd
        ca.LayoutOrder = nO()
    ...
    Se("Pet Hatching")
    Tg("Auto Hatch Egg x50", "AutoHatchEgg", function(enabled)
        Feature.toggleAutoHatchEgg(enabled)
    end, "rbxassetid://93830334871541")
    
    Se("Gacha Spin")
    In("Spin Amount", "SpinAmount")
