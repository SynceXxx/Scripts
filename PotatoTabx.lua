...
local function showConfirmDialog()
    ...
    local function applyTheme(theme)
        ...
        -- =====================================================
        -- FIX 1: Simpan OLD colors SEBELUM Co diupdate
        -- Supaya perbandingan di updateElement masih akurat
        -- =====================================================
        local oldCo = {
            bg  = Co.bg,
            cd  = Co.cd,
        ...
        Co.ac  = colors.ac
        Co.ach = colors.ach
        -- sc (section text) = turunan dari ts, sedikit lebih redup
        ...
        -- Simpan colors aktif ke S supaya notification system bisa pakai
        S.CurrentColors = colors

        -- Helper: cek warna mirip (tolerance karena floating point)
        ...
        -- =====================================================
        -- FIX 2: updateElement pakai oldCo untuk perbandingan
        -- FIX 3: deteksi toggle ON/OFF pakai oldCo.ac, bukan hardcode biru
        -- =====================================================
        local function updateElement(element)
            ...
            pcall(function()
                ...
                elseif element:IsA("TextLabel") then
                    local tc = element.TextColor3
                    if colorMatch(tc, oldCo.tx) then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    elseif colorMatch(tc, oldCo.ts) then
                        T:Create(element, tweenInfo, {TextColor3 = colors.ts}):Play()
                    elseif colorMatch(tc, oldCo.ac) then
                        T:Create(element, tweenInfo, {TextColor3 = colors.ac}):Play()
                    elseif colorMatch(tc, oldCo.sc) then
                        -- Section header text (AUTO CLICK, SETTINGS, dll)
                        T:Create(element, tweenInfo, {TextColor3 = Co.sc}):Play()
                    end
                ...
                elseif element:IsA("TextButton") then
                    ...
                    if isToggle then
                        -- FIX 3: deteksi ON pakai oldCo.ac, OFF pakai oldCo.ch
                        local isOn = colorMatch(bg, oldCo.ac)
                        local isOff = colorMatch(bg, oldCo.ch)
                        if isOn or isOff then
                            T:Create(element, tweenInfo, {
                                BackgroundColor3 = isOn and colors.ac or colors.ch
                            }):Play()
                        end
                    else
                        if colorMatch(bg, oldCo.cd) then
                            T:Create(element, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                        elseif colorMatch(bg, oldCo.ch) then
                            T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                        elseif colorMatch(bg, oldCo.ac) then
                ...
                elseif element:IsA("TextBox") then
                    local bg = element.BackgroundColor3
                    if colorMatch(bg, oldCo.ch) then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                    end
                    if colorMatch(element.TextColor3, oldCo.tx) then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    end
                    if colorMatch(element.PlaceholderColor3, oldCo.ts) then
                        T:Create(element, tweenInfo, {PlaceholderColor3 = colors.ts}):Play()
                    end

                elseif element:IsA("ImageLabel") or element:IsA("ImageButton") then
                    local ic = element.ImageColor3
                    if colorMatch(ic, oldCo.ts) then
                        T:Create(element, tweenInfo, {ImageColor3 = colors.ts}):Play()
                    elseif colorMatch(ic, oldCo.tx) then
                        T:Create(element, tweenInfo, {ImageColor3 = colors.tx}):Play()
                    elseif colorMatch(ic, oldCo.ac) then
                        T:Create(element, tweenInfo, {ImageColor3 = colors.ac}):Play()
                    end
                end
            end)
        ...
        if m then updateElement(m) end

        -- Update divider header
        if dv then
            T:Create(dv, tweenInfo, {BackgroundColor3 = colors.br}):Play()
        end

        -- Update mobile button jika ada
        if S.Mb then
            T:Create(S.Mb, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
            local mbStroke = S.Mb:FindFirstChildOfClass("UIStroke")
            if mbStroke then
                T:Create(mbStroke, tweenInfo, {Color = colors.br}):Play()
            end
            local mbText = S.Mb:FindFirstChildOfClass("TextLabel")
            if mbText then
                T:Create(mbText, tweenInfo, {TextColor3 = colors.tx}):Play()
            end
            local mbIcon = S.Mb:FindFirstChildOfClass("ImageLabel")
            if mbIcon then
                T:Create(mbIcon, tweenInfo, {ImageColor3 = colors.tx}):Play()
            end
        end
