...
local function showConfirmDialog()
    ...
    local function applyTheme(theme)
        ...
        -- Main frame
        if m then
            T:Create(m, tweenInfo, {BackgroundColor3 = colors.bg}):Play()
        end
        if hd then
            T:Create(hd, tweenInfo, {BackgroundColor3 = colors.bg}):Play()
        end
        
        -- Update stroke
        if m and m:FindFirstChild("UIStroke") then
            T:Create(m.UIStroke, tweenInfo, {Color = colors.br}):Play()
        end
        
        -- Header text
        if ttl then
            T:Create(ttl, tweenInfo, {TextColor3 = colors.tx}):Play()
        end
        if vr then
            T:Create(vr, tweenInfo, {TextColor3 = colors.ts}):Play()
        end
        
        -- Buttons
        if clBtn then
            T:Create(clBtn, tweenInfo, {BackgroundColor3 = colors.ch, TextColor3 = colors.ts}):Play()
        end
        if cb then
            T:Create(cb, tweenInfo, {BackgroundColor3 = colors.ch, TextColor3 = colors.ts}):Play()
        end
        if themeBtn then
            T:Create(themeBtn, tweenInfo, {BackgroundColor3 = colors.ch, TextColor3 = colors.ts}):Play()
        end
        
        -- Divider
        if dv then
            T:Create(dv, tweenInfo, {BackgroundColor3 = colors.br}):Play()
        end
        
        -- Update dropdown
        if themeDropdown then
            T:Create(themeDropdown, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
            if themeDropdown:FindFirstChild("UIStroke") then
                T:Create(themeDropdown.UIStroke, tweenInfo, {Color = colors.br}):Play()
            end
        end
        ...
        local function updateElement(element)
            ...
            pcall(function()
                if element:IsA("Frame") or element:IsA("ScrollingFrame") then
                    if element.BackgroundColor3 == Co.cd or element.Name:match("Card") or element.Name:match("Section") then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                    end
                    if element:FindFirstChild("UIStroke") then
                        T:Create(element.UIStroke, tweenInfo, {Color = colors.br}):Play()
                    end
                elseif element:IsA("TextLabel") then
                    if element.Name:match("Title") or element.Name:match("Label") then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    elseif element.Name:match("Desc") or element.Name:match("Subtitle") then
                        T:Create(element, tweenInfo, {TextColor3 = colors.ts}):Play()
                    end
                elseif element:IsA("TextButton") then
                    if not element.Name:match("Close") and not element.Name:match("Minimize") and not element.Name:match("Theme") then
