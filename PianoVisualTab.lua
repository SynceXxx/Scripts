...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName)
    ...
    local w = S.w or 480  -- Landscape width from state
    local h = S.h or 320  -- Landscape height from state
...
local function showConfirmDialog()
    ...
    confirmBtn.MouseButton1Click:Connect(function()
        ...
        -- RESET ALL STATE BEFORE DESTROY
        C.CurrentSheet = ""
        C.SongName = ""
        C.SelectedSong = "::Select Song::"
        C.BPM = 160
        C.IsPlaying = false
        C.CurrentPage = "editor"
        
        -- Destroy UI
        S.U = true
        _G.SyncePianoLoaded = false
        ...
        print("SynceHub Piano closed - all state reset!")
    end)
end
    ...
    -- Slider creation function (with optional icon support)
    local function Sl(n, cK, min, max, callback, ico)
        ...
        if ico then
            local ic = Instance.new("ImageLabel")
            ic.Size = UDim2.new(0, 20, 0, 20)
            ic.Position = UDim2.new(0, 14, 0, 8)
            ic.BackgroundTransparency = 1
        ...
        lb.Position = UDim2.new(0, ico and 42 or 14, 0, 8)  -- Shift right if icon exists
    ...
    local function Ib(n, cK)
        ...
        ib.TextSize = fS
        -- Dynamic placeholder based on field name
        if n:lower():find("name") then
            ib.PlaceholderText = "Enter name..."
        else
        ...
        -- Load nilai awal (handle both number and string)
        if C[cK] then
            if type(C[cK]) == "number" and C[cK] > 0 then
                ib.Text = tostring(C[cK])
            elseif type(C[cK]) == "string" and C[cK] ~= "" then
                ib.Text = C[cK]
            end
        end
        ...
        ib.FocusLost:Connect(function()
            -- If it's a name field, just save as string
            if n:lower():find("name") then
                C[cK] = ib.Text
                return
            end
    ...
    local function Dd(n, op, cK)
        -- Store function reference for refresh
        local opFunc = type(op) == "function" and op or nil
        -- Call function to get initial options
        ...
        local dN = D[C[cK]] or C[cK] or "Select..."
        ...
        db.MouseButton1Click:Connect(function()
            ...
            cD()
            if not wV then
                -- REFRESH: Get fresh options if it's a function (for dynamic library dropdown)
                if opFunc then
                    op = opFunc()
                    buildOptions()  -- Rebuild all options with fresh data
                    -- Update canvas size
                    dS.CanvasSize = UDim2.new(0, 0, 0, (#op * 40) + (#op - 1) * 2 + 12)
    ...
    -- Build UI content for Piano Edition
    Se("Sheet Editor")
    
    -- Multi-line TextBox for sheet (FIX OVERFLOW BUG)
    local editorBox = Instance.new("TextBox")
    editorBox.Size = UDim2.new(1, 0, 0, 150)
    editorBox.BackgroundColor3 = Co.cd
    editorBox.Text = C.CurrentSheet or ""
    editorBox.PlaceholderText = "No sheet loaded yet.\nPaste your sheet here!"
    editorBox.PlaceholderColor3 = Co.ts
    editorBox.MultiLine = true
    editorBox.TextWrapped = true
    editorBox.TextColor3 = Co.tx
    editorBox.Font = Enum.Font.GothamMedium
    editorBox.TextSize = fS
    editorBox.TextXAlignment = Enum.TextXAlignment.Center
    editorBox.TextYAlignment = Enum.TextYAlignment.Center
    editorBox.ClearTextOnFocus = false
    editorBox.ClipsDescendants = true  -- FIX: Prevent text overflow
    editorBox.LayoutOrder = nO()
    editorBox.Parent = sc
    
    local edCorner = Instance.new("UICorner", editorBox)
    edCorner.CornerRadius = UDim.new(0, 12)
    local edStroke = Instance.new("UIStroke", editorBox)
    edStroke.Color = Co.br
    
    local edPad = Instance.new("UIPadding", editorBox)
    edPad.PaddingLeft = UDim.new(0, 8)
    edPad.PaddingRight = UDim.new(0, 8)
    edPad.PaddingTop = UDim.new(0, 8)
    edPad.PaddingBottom = UDim.new(0, 8)
    
    editorBox:GetPropertyChangedSignal("Text"):Connect(function()
        C.CurrentSheet = editorBox.Text
        
        -- Auto switch alignment when typing (UX improvement)
    ...
    -- Initialize CurrentSheet if nil
    if not C.CurrentSheet then
        C.CurrentSheet = ""
    end
    
    -- BPM Control row (dengan icon, tanpa section header)
    Sl("BPM Control", "BPM", 50, 1200, nil, "132758965418868")
    
    -- Performance container (dengan icon, tanpa section header)
    local playbackContainer = Instance.new("Frame")
    playbackContainer.Size = UDim2.new(1, 0, 0, bH)
    playbackContainer.BackgroundColor3 = Co.cd
    playbackContainer.LayoutOrder = nO()
    playbackContainer.Parent = sc
    
    Instance.new("UICorner", playbackContainer).CornerRadius = UDim.new(0, 12)
    Instance.new("UIStroke", playbackContainer).Color = Co.br
    
    -- Icon piano untuk Become a pianist
    local perfIcon = Instance.new("ImageLabel")
    perfIcon.Size = UDim2.new(0, 20, 0, 20)
    perfIcon.Position = UDim2.new(0, 14, 0.5, -10)
    perfIcon.BackgroundTransparency = 1
    perfIcon.Image = "rbxassetid://128722297185969"  -- Icon piano
    perfIcon.ImageColor3 = Co.ts
    perfIcon.Parent = playbackContainer
    
    -- Label "Become a pianist"
    local playLabel = Instance.new("TextLabel")
    playLabel.Size = UDim2.new(0.48, 0, 1, 0)
    playLabel.Position = UDim2.new(0, 42, 0, 0)  -- Geser ke kanan karena ada icon
    playLabel.BackgroundTransparency = 1
    playLabel.Text = "Become a pianist"
    playLabel.TextColor3 = Co.tx
    playLabel.Font = Enum.Font.GothamBold
    playLabel.TextSize = fS
    playLabel.TextXAlignment = Enum.TextXAlignment.Left
    playLabel.TextYAlignment = Enum.TextYAlignment.Center
    playLabel.Parent = playbackContainer
    
    -- Play Button dengan Icon + Text (Grouped Center)
    local playBtn = Instance.new("TextButton")
    playBtn.Size = UDim2.new(0, 78, 0, 32)
    playBtn.Position = UDim2.new(1, -168, 0.5, -16)
    playBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
    playBtn.Text = ""
    playBtn.Parent = playbackContainer
    
    Instance.new("UICorner", playBtn).CornerRadius = UDim.new(0, 8)
    
    -- Container untuk group icon + text di tengah
    local playGroup = Instance.new("Frame")
    playGroup.Size = UDim2.new(1, 0, 1, 0)
    playGroup.Position = UDim2.new(0, 0, 0, 0)
    playGroup.BackgroundTransparency = 1
    playGroup.Parent = playBtn
    
    -- Play Icon
    local playIcon = Instance.new("ImageLabel")
    playIcon.Size = UDim2.new(0, 14, 0, 14)
    playIcon.Position = UDim2.new(0.5, -28, 0.5, -7)  -- Grouped center
    playIcon.BackgroundTransparency = 1
    playIcon.Image = "rbxassetid://132709477378346"  -- Play icon
    playIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
    playIcon.Parent = playGroup
    
    -- Play Text
    local playText = Instance.new("TextLabel")
    playText.Size = UDim2.new(0, 35, 1, 0)
    playText.Position = UDim2.new(0.5, -10, 0, 0)  -- Grouped center
    playText.BackgroundTransparency = 1
    playText.Text = " Play"
    playText.TextColor3 = Color3.fromRGB(255, 255, 255)
    playText.Font = Enum.Font.GothamBold
    playText.TextSize = fS - 1
    playText.TextXAlignment = Enum.TextXAlignment.Left
    playText.TextYAlignment = Enum.TextYAlignment.Center
    playText.Parent = playGroup
    
    -- 3D Effect for Play
    local playStroke = Instance.new("UIStroke", playBtn)
    playStroke.Color = Color3.fromRGB(80, 80, 90)
    ...
    playBtn.MouseButton1Click:Connect(function()
        if not C.IsPlaying then
            playSound(Sounds.Click, 0.4)
            local success = Feature.togglePlayback(true)
            if success then
                C.IsPlaying = true
            end
        end
    ...
    -- Monitor C.IsPlaying to reset button when music finishes (NO VISUAL CHANGE)
    task.spawn(function()
    ...
    -- Stop Button dengan Icon + Text (Grouped Center)
    local stopBtn = Instance.new("TextButton")
    stopBtn.Size = UDim2.new(0, 78, 0, 32)
    stopBtn.Position = UDim2.new(1, -85, 0.5, -16)
    stopBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
    stopBtn.Text = ""
    stopBtn.Parent = playbackContainer
    
    Instance.new("UICorner", stopBtn).CornerRadius = UDim.new(0, 8)
    
    -- Container untuk group icon + text di tengah
    local stopGroup = Instance.new("Frame")
    stopGroup.Size = UDim2.new(1, 0, 1, 0)
    stopGroup.Position = UDim2.new(0, 0, 0, 0)
    stopGroup.BackgroundTransparency = 1
    stopGroup.Parent = stopBtn
    
    -- Stop Icon
    local stopIcon = Instance.new("ImageLabel")
    stopIcon.Size = UDim2.new(0, 14, 0, 14)
    stopIcon.Position = UDim2.new(0.5, -28, 0.5, -7)  -- Grouped center
    stopIcon.BackgroundTransparency = 1
    stopIcon.Image = "rbxassetid://121286958936752"  -- Stop icon
    stopIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
    stopIcon.Parent = stopGroup
    
    -- Stop Text
    local stopText = Instance.new("TextLabel")
    stopText.Size = UDim2.new(0, 35, 1, 0)
    stopText.Position = UDim2.new(0.5, -10, 0, 0)  -- Grouped center
    stopText.BackgroundTransparency = 1
    stopText.Text = " Stop"
    stopText.TextColor3 = Color3.fromRGB(255, 255, 255)
    stopText.Font = Enum.Font.GothamBold
    stopText.TextSize = fS - 1
    stopText.TextXAlignment = Enum.TextXAlignment.Left
    stopText.TextYAlignment = Enum.TextYAlignment.Center
    stopText.Parent = stopGroup
    
    -- 3D Effect for Stop
    local stopStroke = Instance.new("UIStroke", stopBtn)
    stopStroke.Color = Color3.fromRGB(90, 70, 75)
    ...
    stopBtn.MouseButton1Click:Connect(function()
        if C.IsPlaying then
            playSound(Sounds.Click, 0.3)
            Feature.togglePlayback(false)
            C.IsPlaying = false
            -- Reset Play button visual
            playBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
        end
    end)
    
    stopBtn.MouseEnter:Connect(function()
        T:Create(stopBtn, TweenInfo.new(0.2), {
            BackgroundColor3 = Color3.fromRGB(70, 50, 55)
        }):Play()
        T:Create(stopStroke, TweenInfo.new(0.2), {
            Transparency = 0.1
        }):Play()
    end)
    ...
    -- Song List Container with dynamic buttons
    local songListContainer = Instance.new("Frame")
    songListContainer.Name = "SongListContainer"
    songListContainer.Size = UDim2.new(1, 0, 0, 200)
    songListContainer.BackgroundColor3 = Co.cd
    songListContainer.BorderSizePixel = 0
    songListContainer.LayoutOrder = nO()
    songListContainer.ClipsDescendants = true
    songListContainer.Parent = sc
    
    Instance.new("UICorner", songListContainer).CornerRadius = UDim.new(0, 12)
    Instance.new("UIStroke", songListContainer).Color = Co.br
    
    -- Scrolling frame for songs
    local songScroll = Instance.new("ScrollingFrame")
    songScroll.Size = UDim2.new(1, 0, 1, 0)
    songScroll.BackgroundTransparency = 1
    songScroll.ScrollBarThickness = 4
    songScroll.ScrollBarImageColor3 = Co.br
    songScroll.BorderSizePixel = 0
    songScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
    songScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
    songScroll.Parent = songListContainer
    
    local songPadding = Instance.new("UIPadding", songScroll)
    songPadding.PaddingLeft = UDim.new(0, 8)
    songPadding.PaddingRight = UDim.new(0, 8)
    songPadding.PaddingTop = UDim.new(0, 8)
    songPadding.PaddingBottom = UDim.new(0, 8)
    
    local songLayout = Instance.new("UIListLayout", songScroll)
    songLayout.Padding = UDim.new(0, 6)
    songLayout.SortOrder = Enum.SortOrder.LayoutOrder
    
    -- Function to refresh song list
    local function refreshSongList()
        -- Clear existing items (Frame AND TextLabel)
        for _, child in ipairs(songScroll:GetChildren()) do
            if child:IsA("Frame") or child:IsA("TextLabel") then
                child:Destroy()
            end
        end
        
        local songs = Feature.getLibrary()
        ...
        if #songs == 0 then
            ...
            emptyLabel.TextColor3 = Co.ts
            emptyLabel.Font = Enum.Font.GothamMedium
            emptyLabel.TextSize = fS
            emptyLabel.TextWrapped = true
            emptyLabel.Parent = songScroll
        else
            -- Create song items
            for i, song in ipairs(songs) do
                local songIndex = i  -- Capture index in local variable for closure safety
                local songName = song.Name  -- Capture name too
                
                local songItem = Instance.new("Frame")
                songItem.Size = UDim2.new(1, 0, 0, 44)
                songItem.BackgroundColor3 = Co.ch
                songItem.BorderSizePixel = 0
                songItem.LayoutOrder = songIndex
                songItem.Parent = songScroll
                
                Instance.new("UICorner", songItem).CornerRadius = UDim.new(0, 10)
                ...
                nameLabel.TextColor3 = Co.tx
                nameLabel.Font = Enum.Font.GothamBold
                nameLabel.TextSize = fS
                nameLabel.TextXAlignment = Enum.TextXAlignment.Left
                nameLabel.TextTruncate = Enum.TextTruncate.AtEnd
                nameLabel.Parent = songItem
                
                -- Load button (Ukuran sama dengan Play/Stop)
                local loadBtn = Instance.new("TextButton")
                loadBtn.Size = UDim2.new(0, 78, 0, 32)  -- SAMA dengan Play/Stop
                loadBtn.Position = UDim2.new(1, -166, 0.5, -16)  -- Precise position
                loadBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
                loadBtn.Text = ""
                loadBtn.Parent = songItem
                
                Instance.new("UICorner", loadBtn).CornerRadius = UDim.new(0, 8)
                
                -- Container untuk group icon + text di tengah
                local loadGroup = Instance.new("Frame")
                loadGroup.Size = UDim2.new(1, 0, 1, 0)
                loadGroup.Position = UDim2.new(0, 0, 0, 0)
                loadGroup.BackgroundTransparency = 1
                loadGroup.Parent = loadBtn
                
                -- Load Icon
                local loadIcon = Instance.new("ImageLabel")
                loadIcon.Size = UDim2.new(0, 14, 0, 14)
                loadIcon.Position = UDim2.new(0.5, -30, 0.5, -7)  -- Grouped center
                loadIcon.BackgroundTransparency = 1
                loadIcon.Image = "rbxassetid://73647534164545"  -- Load icon
                loadIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
                loadIcon.Parent = loadGroup
                
                -- Load Text
                local loadText = Instance.new("TextLabel")
                loadText.Size = UDim2.new(0, 38, 1, 0)
                loadText.Position = UDim2.new(0.5, -12, 0, 0)  -- Grouped center
                loadText.BackgroundTransparency = 1
                loadText.Text = "Load"
                loadText.TextColor3 = Color3.fromRGB(255, 255, 255)
                loadText.Font = Enum.Font.GothamBold
                loadText.TextSize = fS - 1
                loadText.TextXAlignment = Enum.TextXAlignment.Left
                loadText.TextYAlignment = Enum.TextYAlignment.Center
                loadText.Parent = loadGroup
                
                loadBtn.MouseButton1Click:Connect(function()
                    playSound(Sounds.Click, 0.4)
                    local sheet = Feature.loadSong(songIndex)  -- Use captured index
                    if sheet then
                        C.CurrentSheet = sheet
                        -- Update editor box
                        local edBox = sc:FindFirstChild("TextBox", true)
                        if edBox then
                            edBox.Text = sheet
                        end
                        Feature.showNotification("Loaded: " .. songName, true)  -- Use captured name
                    end
                end)
                
                -- Delete button (Ukuran sama dengan Play/Stop)
                local delBtn = Instance.new("TextButton")
                delBtn.Size = UDim2.new(0, 78, 0, 32)  -- SAMA dengan Play/Stop
                delBtn.Position = UDim2.new(1, -83, 0.5, -16)  -- 5px gap dari Load
                delBtn.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
                delBtn.Text = ""
                delBtn.Parent = songItem
                
                Instance.new("UICorner", delBtn).CornerRadius = UDim.new(0, 8)
                
                -- Container untuk group icon + text di tengah
                local delGroup = Instance.new("Frame")
                delGroup.Size = UDim2.new(1, 0, 1, 0)
                delGroup.Position = UDim2.new(0, 0, 0, 0)
                delGroup.BackgroundTransparency = 1
                delGroup.Parent = delBtn
                
                -- Delete Icon
                local delIcon = Instance.new("ImageLabel")
                delIcon.Size = UDim2.new(0, 14, 0, 14)
                delIcon.Position = UDim2.new(0.5, -26, 0.5, -7)  -- Grouped center
                delIcon.BackgroundTransparency = 1
                delIcon.Image = "rbxassetid://119532706729140"  -- Delete icon
                delIcon.ImageColor3 = Color3.fromRGB(255, 255, 255)
                delIcon.Parent = delGroup
                
                -- Delete Text
                local delText = Instance.new("TextLabel")
                delText.Size = UDim2.new(0, 30, 1, 0)
                delText.Position = UDim2.new(0.5, -8, 0, 0)  -- Grouped center
                delText.BackgroundTransparency = 1
                delText.Text = "Delete"
                delText.TextColor3 = Color3.fromRGB(255, 255, 255)
                delText.Font = Enum.Font.GothamBold
                delText.TextSize = fS - 1
                delText.TextXAlignment = Enum.TextXAlignment.Left
                delText.TextYAlignment = Enum.TextYAlignment.Center
                delText.Parent = delGroup
                
                delBtn.MouseButton1Click:Connect(function()
                    playSound(Sounds.Click, 0.3)
                    Feature.deleteSong(songIndex)  -- Use captured index
                    refreshSongList()  -- Refresh after delete
                end)
            end
        end
    end
    ...
    -- Song Name Input
    Ib("Song Name", "SongName")
    
    -- Add Song Button
    Bt("Add to Library", function()
        local success = Feature.addSong()
        if success then
            C.SongName = ""
            refreshSongList()  -- Refresh list after adding
            task.wait(0.3)
            Feature.showNotification("Song list updated!", true)
        end
    end, "rbxassetid://89284204240420")
    ...
    Bt("Destroy UI", function()
        ...
        -- Show cleanup notification (brief)
        print("SynceHub Piano closed - all state reset!")
