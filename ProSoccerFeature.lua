...
-- Feature 4: Auto Collect Energy (Auto-detect all UUIDs)
function Feature.toggleAutoCollectEnergy(enabled)
    if enabled then
        if Connections.AutoCollect then return end
        
        Connections.AutoCollect = R.Heartbeat:Connect(function()
            pcall(function()
                local Event = RS:FindFirstChild("Events")
                ...
                -- Method 1: Find collectables in Workspace
                local function collectFromWorkspace()
                    for _, obj in pairs(W:GetDescendants()) do
                        if obj.Name == "Energy" or obj.Name == "Collectable" or obj.Name == "Orb" then
                            -- Check if object has unique ID (UUID)
                            local id = obj:GetAttribute("Id") or obj:GetAttribute("ID") or obj:GetAttribute("uuid") or obj.Name
                ...
                -- Method 2: Try collecting from a specific folder
                local collectFolder = W:FindFirstChild("Collectables") or W:FindFirstChild("Energy") or W:FindFirstChild("Orbs")
                if collectFolder then
                    for _, item in pairs(collectFolder:GetChildren()) do
                        local id = item:GetAttribute("Id") or item:GetAttribute("ID") or item:GetAttribute("uuid") or item.Name
                        if id and typeof(id) == "string" then
                            Event.Collectable.Collected:FireServer(id)
                        end
                    end
        ...
        Feature.showNotification("Auto Collect Energy enabled!", true)
    else
