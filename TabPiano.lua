...
local function showConfirmDialog()
    ...
    -- Load Song Dropdown
    Dd("Load Saved Song", function()
        local songs = Feature.getLibrary()
        local options = {"::Select Song::"}
        for _, song in ipairs(songs) do
            table.insert(options, song.Name)
        end
        return options
    end, "SelectedSong")
    
    -- Watch for dropdown changes and load song
    local lastSelected = C.SelectedSong
    task.spawn(function()
        while S.G do
            if C.SelectedSong and C.SelectedSong ~= lastSelected and C.SelectedSong ~= "::Select Song::" then
                lastSelected = C.SelectedSong
                -- Find song by name and load it
                local songs = Feature.getLibrary()
                for i, song in ipairs(songs) do
                    if song.Name == C.SelectedSong then
                        local sheet = Feature.loadSong(i)
                        if sheet then
                            C.CurrentSheet = sheet
                            -- Update editor box text manually
    ...
    Bt("Add to Library", function()
        ...
        if success then
            C.SongName = ""
            -- Additional notification for user guidance
            task.wait(0.5)
            Feature.showNotification("Open dropdown to see your saved song!", true)
        end
