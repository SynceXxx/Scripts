...
-- Knit Services (Dig Legends uses Knit framework)
local Knit = nil
local Services = {
    FightService = nil,
...
function Feature.init(config, state)
    ...
    if not Knit then
        warn("Knit framework not found! Some features may not work.")
    end
end
...
-- Auto Rewards (Loop claim all online rewards)
...
-- Claim Starter Pack (One-time claim)
