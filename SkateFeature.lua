...
-- Feature 8: Auto Hatch Egg x50 (Toggle)
function Feature.toggleAutoHatchEgg(enabled)
    if enabled then
        if Connections.AutoHatchEgg then return end
        
        Connections.AutoHatchEgg = R.Heartbeat:Connect(function()
            task.wait(0.5)
            pcall(function()
                local Event = RS:FindFirstChild("RemoteFunction")
                if Event then
        ...
        Feature.showNotification("Auto Hatch Egg x50 enabled!", true)
    else
        if Connections.AutoHatchEgg then
            Connections.AutoHatchEgg:Disconnect()
            Connections.AutoHatchEgg = nil
        end
        Feature.showNotification("Auto Hatch Egg x50 disabled!", false)
    end
end
