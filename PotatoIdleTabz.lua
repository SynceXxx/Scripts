...
local function showConfirmDialog()
    ...
    -- ============================================
    -- SIDEBAR + TOGGLE (child of m, posisi negatif di kiri)
    -- m.ClipsDescendants = false supaya bisa keluar batas frame
    -- Otomatis ngikut drag, tidak perlu RenderStepped
    -- ============================================
    local sidebarW    = 52
    local sidebarOpen = false

    -- Sidebar frame — child of m, posisi di kiri luar hub
    local sidebar = Instance.new("Frame")
    sidebar.Name = "Sidebar"
    sidebar.Size = UDim2.new(0, sidebarW, 1, 0)  -- tinggi sama dengan hub
    sidebar.AnchorPoint = Vector2.new(1, 0)
    -- Saat tertutup: geser ke kanan sampai tersembunyi di balik hub edge
    sidebar.Position = UDim2.new(0, 0, 0, 0)
    sidebar.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    sidebar.BorderSizePixel = 0
    sidebar.ZIndex = 1  -- di belakang hub (hub ZIndex default lebih tinggi)
    sidebar.Visible = false
    ...
    -- Toggle button "OPEN" — child of m, nempel di kiri luar hub
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, 18, 0, 60)
    toggleBtn.AnchorPoint = Vector2.new(1, 0.5)
    toggleBtn.Position = UDim2.new(0, -4, 0.5, 0)  -- nempel di kiri luar hub, tengah vertikal
    toggleBtn.BackgroundColor3 = Co.cd
    toggleBtn.Text = ""
    toggleBtn.ZIndex = 2
    toggleBtn.AutoButtonColor = false
    toggleBtn.Parent = m  -- child of m, ikut drag otomatis
    ...
    -- Saat tertutup: sidebar.Position.X = 0 (tersembunyi di balik kiri hub)
    -- Saat terbuka:  sidebar.Position.X = -(sidebarW + gap)
    local GAP = 6
        ...
        mT.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if not wasDragged then
                    ...
                    -- Sidebar & toggle ikut hide/show
                    toggleBtn.Visible = m.Visible
                    if not m.Visible then
                        -- Tutup sidebar saat hub di-hide
                        sidebar.Visible = false
                        sidebarOpen = false
                        toggleLabel.Text = "OPEN"
                        toggleLabel.TextColor3 = Co.ts
