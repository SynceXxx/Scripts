...
function Feature.togglePlayback(enabled)
    if enabled then
        if Connections.Playback then 
            ...
            pcall(function() task.cancel(Connections.Playback) end)
            Connections.Playback = nil
        end
        
        if not C.CurrentSheet or C.CurrentSheet == "" then
        ...
        Connections.Playback = task.spawn(function()
            ...
            -- CRITICAL: Clear connection and reset state
            C.IsPlaying = false
            Connections.Playback = nil
...
function Feature.addSong()
    if not C.SongName or C.SongName == "" or not C.CurrentSheet or C.CurrentSheet == "" then
