-- Features.lua - Skateboard Training
local Feature = {}
    ...
    if enabled then
        ...
        Connections.TicketLuckyWheel = R.Heartbeat:Connect(function()
            ...
            pcall(function()
                ...
                if Event then
                    Event:InvokeServer("lucky_wheel_get_ticket", {["1"] = 6})
                end
            end)
        end)
...
function Feature.opAutoTrain()
    pcall(function()
        ...
        if Event then
            Event:FireServer("train_start", true, {["1"] = 42})
    ...
    if enabled then
        ...
        Connections.AutoSpin = R.Heartbeat:Connect(function()
            ...
            pcall(function()
                local Event = RS:FindFirstChild("RemoteFunction")
                if Event then
                    Event:InvokeServer("client_daily_gacha_reward", {["1"] = C.SpinAmount})
                end
            end)
        end)
