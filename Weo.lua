...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    for i, def in ipairs(tabDefs) do
        ...
        -- 3D shadow bawah (kedalaman)
        local tabShadow = Instance.new("Frame")
        tabShadow.Size             = UDim2.new(1, 0, 1, 3)
        tabShadow.Position         = UDim2.new(0, 0, 0, 3)
        tabShadow.BackgroundColor3 = isActive and Color3.fromRGB(90, 40, 180) or Color3.fromRGB(20, 12, 45)
        tabShadow.BorderSizePixel  = 0
        tabShadow.ZIndex           = 3
        tabShadow.Parent           = tabBtn
        Instance.new("UICorner", tabShadow).CornerRadius = UDim.new(0, 8)

        -- Stroke biar keliatan timbul
        local tabStroke = Instance.new("UIStroke", tabBtn)
        tabStroke.Color       = isActive and Color3.fromRGB(180, 120, 255) or Color3.fromRGB(60, 35, 100)
        ...
        tabButtons[i] = { btn = tabBtn, icon = tabIcon, shadow = tabShadow, stroke = tabStroke }
    end
    ...
    local function switchTab(index)
        ...
        for i, tb in ipairs(tabButtons) do
            ...
            if tb.stroke then
                T:Create(tb.stroke, TweenInfo.new(0.2), {
                    Color = isActive and Color3.fromRGB(180, 120, 255) or Color3.fromRGB(60, 35, 100)
                }):Play()
            end
            if tb.shadow then
                T:Create(tb.shadow, TweenInfo.new(0.2), {
                    BackgroundColor3 = isActive and Color3.fromRGB(90, 40, 180) or Color3.fromRGB(20, 12, 45)
                }):Play()
            end
    ...
    -- Connect tab buttons
    for i, tb in ipairs(tabButtons) do
        -- Press effect (InputBegan)
        tb.btn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or
               input.UserInputType == Enum.UserInputType.Touch then
                T:Create(tb.btn, TweenInfo.new(0.08, Enum.EasingStyle.Quad), {
                    Size = UDim2.new(1, -4, 0, SB_BTN_SIZE - 4)
                }):Play()
                if tb.shadow then
                    T:Create(tb.shadow, TweenInfo.new(0.08), {
        ...
        -- Release effect (InputEnded)
        tb.btn.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or
               input.UserInputType == Enum.UserInputType.Touch then
                T:Create(tb.btn, TweenInfo.new(0.15, Enum.EasingStyle.Back), {
                    Size = UDim2.new(1, 0, 0, SB_BTN_SIZE)
                }):Play()
                if tb.shadow then
                    T:Create(tb.shadow, TweenInfo.new(0.15), {
