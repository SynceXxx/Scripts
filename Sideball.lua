...
function Sidebar.create(g, m, Co, onTabSwitch)
    ...
    m:GetPropertyChangedSignal("Visible"):Connect(function()
        if not m.Visible then
            -- Sembunyikan pill & sidebar saat UI di-hide
            pill.Visible    = false
            sidebar.Visible = false
        else
            if pillShouldShow then
                task.spawn(function()
                    -- Tunggu beberapa frame sampai AbsolutePosition & AbsoluteSize settle
                    task.wait()
                    task.wait()
                    task.wait()

                    -- Sync posisi sidebar dulu kalau open
                    if sidebarOpen then
                        local mX  = m.AbsolutePosition.X
                        local mY  = m.AbsolutePosition.Y
                        local mH  = m.AbsoluteSize.Y
                    ...
                    -- Panggil showPill() — dia sudah handle semua state (open/close)
    ...
    local function showPill()
        ...
        -- Poll sampai m.AbsoluteSize.Y sudah full (bukan 52 minimize)
        -- Max tunggu 1 detik
        local waited = 0
        while m.AbsoluteSize.Y < 100 and waited < 1 do
            task.wait(0.05)
            waited = waited + 0.05
        ...
        -- Tunggu 1 frame extra biar position juga settle
        task.wait()
        if myId ~= pillShowId then return end
        if not pillShouldShow then return end
        ...
        -- Kalau sidebar open, pakai posisi real sidebar yang sudah final
        local pillX
        if sidebarOpen and sidebar.Visible then
            pillX = sidebar.AbsolutePosition.X - PILL_W + PILL_OVERLAP
        else
