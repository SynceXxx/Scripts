...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    -- SIDEBAR CONSTANTS
    -- ============================================
    local SB_CLOSED_W   = 24   -- width when collapsed
    local SB_OPEN_W     = 54   -- width when expanded
    local SB_OVERLAP    = 12   -- px sidebar hides behind m's left edge
    local SB_Y_OFFSET   = 10   -- top offset relative to m
    local SB_BTN_SIZE   = 38   -- tab icon button size
    local SB_BTN_GAP    = 6    -- gap between tab buttons
    local SB_BTN_MARGIN = 8    -- margin from top of sidebar

    local sidebarOpen      = false
    local sidebarAnimating = false
    local activeTabIndex   = 1

    -- Tab definitions (icon = rbxassetid string)
    local tabDefs = {
    ...
    -- ============================================
    -- MAIN FRAME  (ZIndex 2 → renders on top of sidebar)
    -- ============================================
    ...
    -- Gradient Neon Purple (bg putih dulu biar gradient keliatan)
    ...
    -- ============================================
    -- SIDEBAR  (ZIndex 1 → behind m)
    -- ============================================
    local sidebar = Instance.new("Frame")
    sidebar.Name           = "Sidebar"
    sidebar.Size           = UDim2.new(0, SB_CLOSED_W, 0, h - SB_Y_OFFSET * 2)
    sidebar.Position       = UDim2.new(0, 0, 0, 0)   -- synced below
    ...
    -- "OPEN" label (rotated, shown when collapsed, centered vertically)
    local openLabel = Instance.new("TextLabel")
    openLabel.Name             = "OpenLabel"
    openLabel.Size             = UDim2.new(0, 60, 0, SB_CLOSED_W - 2)
    openLabel.AnchorPoint      = Vector2.new(0.5, 0.5)
    openLabel.Position         = UDim2.new(0.5, 0, 0.5, 0)
    openLabel.BackgroundTransparency = 1
    openLabel.Text             = "OPEN"
    openLabel.TextColor3       = Co.ts
    openLabel.Font             = Enum.Font.GothamBold
    openLabel.TextSize         = 10
    openLabel.Rotation         = 90
    openLabel.ZIndex           = 3
    openLabel:SetAttribute("ThemeTag", "ts")
    openLabel.Parent           = sidebar

    -- Tab buttons container (visible when open)
    ...
    tabBtnContainer.Position         = UDim2.new(0, 4, 0, SB_BTN_MARGIN)
    tabBtnContainer.BackgroundTransparency = 1
    tabBtnContainer.ZIndex           = 3
    tabBtnContainer.Visible          = false
    ...
    -- Full-sidebar clickable area — ZIndex 1 supaya tab buttons (ZIndex 4 via container ZIndex 3) selalu dapat click duluan
    local sidebarToggleBtn = Instance.new("TextButton")
    sidebarToggleBtn.Size              = UDim2.new(1, 0, 1, 0)
    sidebarToggleBtn.BackgroundTransparency = 1
    sidebarToggleBtn.Text              = ""
    sidebarToggleBtn.ZIndex            = 1
    sidebarToggleBtn.Parent            = sidebar

    -- ============================================
    -- SIDEBAR POSITION SYNC (follows m when dragged)
    -- ============================================
    local function syncSidebarPos()
        if not m or not m.Parent then return end
        local ap   = m.AbsolutePosition
        local asSz = m.AbsoluteSize
        local sW   = sidebarOpen and SB_OPEN_W or SB_CLOSED_W
        sidebar.Position = UDim2.new(0, ap.X - sW + SB_OVERLAP, 0, ap.Y + SB_Y_OFFSET)
        sidebar.Size     = UDim2.new(0, sW, 0, asSz.Y - SB_Y_OFFSET * 2)
    end

    -- Sync visibility sidebar mengikuti m
    m:GetPropertyChangedSignal("Visible"):Connect(function()
        sidebar.Visible = m.Visible
    end)
    ...
    local function toggleSidebar()
        ...
        local targetW  = sidebarOpen and SB_OPEN_W or SB_CLOSED_W
        local ap       = m.AbsolutePosition
        local asSz     = m.AbsoluteSize
        local targetX  = ap.X - targetW + SB_OVERLAP
        local targetSzY = asSz.Y - SB_Y_OFFSET * 2

        T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
            Size     = UDim2.new(0, targetW, 0, targetSzY),
            Position = UDim2.new(0, targetX, 0, sidebar.Position.Y.Offset)
        }):Play()

        if sidebarOpen then
            openLabel.Visible          = false
            tabBtnContainer.Visible    = true
            tabBtnContainer.BackgroundTransparency = 1
        else
        ...
        task.delay(0.32, function() sidebarAnimating = false end)
    end
    ...
    sidebarToggleBtn.MouseEnter:Connect(function()
        T:Create(sidebar, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
    end)
    sidebarToggleBtn.MouseLeave:Connect(function()
        T:Create(sidebar, TweenInfo.new(0.15), {BackgroundColor3 = Co.cd}):Play()
    end)
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dS and (i.UserInputType == Enum.UserInputType.MouseMovement or
            ...
            -- Langsung update sidebar tanpa tunggu AbsolutePosition signal
            local sW = sidebarOpen and SB_OPEN_W or SB_CLOSED_W
            local asSz = m.AbsoluteSize
            sidebar.Position = UDim2.new(0, newX - sW + SB_OVERLAP, 0, newY + SB_Y_OFFSET)
            sidebar.Size     = UDim2.new(0, sW, 0, asSz.Y - SB_Y_OFFSET * 2)
        end
    end)
    ...
    -- Initial sidebar position (after g is parented so AbsolutePosition is valid)
    task.wait()
    syncSidebarPos()
