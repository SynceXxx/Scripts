...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    if themesModule and themesModule.List and #themesModule.List > 0 then
        ...
        for i, theme in ipairs(themesModule.List) do
            ...
            -- UIScale untuk press effect (AnchorPoint tengah biar scale dari tengah)
            ...
            -- Stroke: tebal kalau aktif
            local cellStroke = Instance.new("UIStroke", cell)
            cellStroke.Color       = isActive and Color3.fromRGB(255,255,255) or Color3.fromRGB(255,255,255)
            cellStroke.Thickness   = isActive and 3 or 0
            cellStroke.Transparency = isActive and 0 or 1

            -- Inner darker square buat preview bg
            local inner = Instance.new("Frame")
            inner.Size             = UDim2.new(0, 28, 0, 28)
            ...
            cell.MouseButton1Click:Connect(function()
                ...
                for _, ch in ipairs(grid:GetChildren()) do
                    if ch:IsA("TextButton") then
                        ...
                        local chCheck  = ch:FindFirstChildWhichIsA("TextLabel")
                        local sel      = ch == cell
                        if chStroke then
                            T:Create(chStroke, TweenInfo.new(0.2), {
                                Thickness    = sel and 3 or 0,
                                Transparency = sel and 0 or 1,
                            }):Play()
                        end
                        if chCheck then chCheck.Text = sel and "✓" or "" end
                    end
                end
            end)
