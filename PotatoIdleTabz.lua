...
local function showConfirmDialog()
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        cb.Text = mn and "+" or "−"
        -- Sembunyikan sidebar saat minimize
        if mn then
    ...
    -- ============================================
    -- SIDEBAR + TOGGLE
    -- Keduanya child of m (ikut drag otomatis)
    -- Tombol OPEN/CLOSE selalu nempel di sisi kanan sidebar
    -- Sidebar slide kiri/kanan, tombol ikut bareng
    -- ============================================
    local sidebarW    = 52
    local GAP         = 6   -- jarak sidebar ke hub
    local sidebarOpen = false

    -- Sidebar — child of m
    -- Posisi tertutup: x = -(sidebarW+GAP), tapi Visible=false
    -- Posisi terbuka:  x = -(sidebarW+GAP)
    local sidebar = Instance.new("Frame")
    sidebar.Name = "Sidebar"
    sidebar.Size = UDim2.new(0, sidebarW, 1, 0)
    sidebar.AnchorPoint = Vector2.new(1, 0)
    sidebar.Position = UDim2.new(0, -(GAP), 0, 0)  -- mulai tersembunyi (nempel di kiri hub, belum slide)
    ...
    -- Toggle button — child of SIDEBAR bukan m
    -- Selalu nempel di sisi kanan sidebar, otomatis ikut slide bareng sidebar
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, 18, 0, 60)
    toggleBtn.AnchorPoint = Vector2.new(0, 0.5)
    toggleBtn.Position = UDim2.new(1, 2, 0.5, 0)  -- tepat di kanan sidebar
    toggleBtn.BackgroundColor3 = Co.cd
    toggleBtn.Text = ""
    toggleBtn.ZIndex = 3
    toggleBtn.AutoButtonColor = false
    toggleBtn.Parent = sidebar  -- child of sidebar! ikut slide otomatis
    ...
    -- Posisi sidebar:
    -- Tertutup: nempel di kiri hub tapi Visible=false, x = -GAP (tersembunyi di balik hub)
    -- Terbuka:  x = -(sidebarW + GAP), geser keluar ke kiri
    -- Tombol selalu di sisi kanan sidebar, jadi ikut gerak otomatis
        ...
        mT.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                ...
                if not wasDragged then
                    ...
                    -- Sidebar ikut hide/show (toggleBtn sudah child of sidebar)
