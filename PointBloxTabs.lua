-- PointBloxTabs.lua
-- SynceHub - UI Creation (Point Blox ESP)
...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName)
    ...
    local h  = isMobile and math.min(500, sS.Y - 100) or 480
    local bH = isMobile and 46 or 44
    local p  = 12
    local fS = isMobile and 14 or 13

    -- Color scheme (same as original SynceHub)
    local Co = {
    ...
    st.Text               = (gameName or "Point Blox") .. " | " .. version
    st.TextColor3         = Co.ts
    ...
    local function showConfirmDialog()
        ...
        for _, d in ipairs(fadeItems) do
            T:Create(d[1], TweenInfo.new(0.25, Enum.EasingStyle.Quint), {[d[2]] = d[3]}):Play()
        end
        ...
        confirmBtn.MouseButton1Click:Connect(function()
            ...
            fadeOut(function()
                ...
                S.U = true
                _G.SynceHubPointBloxLoaded = false
    ...
    local function Sl(n, min, max, cK, callback)
        ...
        local dG = false
        local function onDrag(inputPos)
            local sp = sB.AbsolutePosition.X
            local sw = sB.AbsoluteSize.X
            local pct = math.clamp((inputPos - sp) / sw, 0, 1)
        ...
        sB.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                dG = true; playSound(Sounds.Click, 0.2); onDrag(i.Position.X)
            end
        end)
    ...
    local oD = {}
    local function cD()
        for _, d in pairs(oD) do if d and d.Visible then d.Visible = false end end
    end
    sc:GetPropertyChangedSignal("CanvasPosition"):Connect(cD)

    local function Dd(n, op, cK, callback)
        local ca = Instance.new("Frame")
        ca.Size               = UDim2.new(1, 0, 0, bH)
        ca.BackgroundColor3   = Co.cd
        ...
        dbText.Size               = UDim2.new(1, -28, 1, 0)
        ...
        dLs.Color = Co.br; dLs.Thickness = 1.5; dLs.Transparency = 0.3
        table.insert(oD, dL)
        ...
        dS.CanvasSize            = UDim2.new(0, 0, 0, #op * 40 + 12)
        dS.ZIndex                = 1000
        dS.Parent                = dL
        ...
        for _, opt in ipairs(op) do
            local ob = Instance.new("TextButton")
            ob.Size            = UDim2.new(1, -8, 0, 40)
            ob.BackgroundColor3 = C[cK] == opt and Co.al or Co.cd
            ob.Text            = ""
            ob.ZIndex          = 1001
            ...
            optText.Text               = opt
            optText.TextColor3         = C[cK] == opt and Co.ac or Co.tx
            optText.Font               = C[cK] == opt and Enum.Font.GothamBold or Enum.Font.GothamMedium
            optText.TextSize           = fS
    ...
    -- Master toggle
    Tg("Player ESP", "ESPEnabled", function(val)
        Feature.toggleESP(val)
    end)

    Se("ESP OPTIONS")

    Tg("Box / Highlight", "ESPBox", function()
        Feature.refreshESP()
    end)

    Tg("Name Tag", "ESPName", function()
        Feature.refreshESP()
    end)

    Tg("Health Info", "ESPHealth", function()
        Feature.refreshESP()
    end)

    Tg("Distance", "ESPDistance", function()
        Feature.refreshESP()
    end)

    Tg("Team Check", "ESPTeamCheck", function()
        Feature.refreshESP()
    end)

    Se("VISUALS")

    -- Fill Transparency slider (0.0 - 1.0, mapped ke 0-10 lalu /10)
    Sl("Fill Opacity", 0, 10, "ESPFillOpacity", function(val)
        C.ESPFillTransparency = 1 - (val / 10)
        Feature.refreshESP()
    end)

    -- Color dropdown
    Dd("ESP Color", {"Red", "Blue", "Green", "Yellow", "Purple", "White"}, "ESPColor", function()
        Feature.refreshESP()
    end)

    Se("ACTIONS")

    Bt("Refresh ESP", function()
        Feature.refreshESP()
        Feature.showNotification("ESP refreshed!", true)
    end, "rbxassetid://131354680051916")
