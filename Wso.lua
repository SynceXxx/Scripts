...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    -- SIDEBAR CONSTANTS
    -- ============================================
    local SB_W          = 48    -- lebar panel sidebar: 40 (btn) + 4+4 padding = tepat kotak 1:1
    ...
    local PILL_OVERLAP  = -4    -- negatif = ada gap antara pill dan sidebar/m (tidak dempet)
    ...
    for i, def in ipairs(tabDefs) do
        ...
        -- Wrapper masuk UIListLayout, ukurannya tidak pernah berubah
        ...
        -- tabBtn di dalam wrapper, AnchorPoint tengah biar scale dari tengah
        ...
        -- UIScale di tabBtn bukan wrapper, layout tidak terpengaruh
    ...
    local function switchTab(index)
        ...
        if index == activeTabIndex then
            return  -- klik tab sama tidak close sidebar
        end
        tabSwitching = true
    ...
    -- Connect tab buttons
    for i, tb in ipairs(tabButtons) do
        -- Press effect (InputBegan) - scale down
        tb.btn.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or
               input.UserInputType == Enum.UserInputType.Touch then
                T:Create(tb.scale, TweenInfo.new(0.08, Enum.EasingStyle.Quad), {
                    Scale = 0.88
                }):Play()
            end
        end)

        -- Release effect (InputEnded) - scale balik dengan bounce
        tb.btn.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or
               input.UserInputType == Enum.UserInputType.Touch then
                T:Create(tb.scale, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Scale = 1
                }):Play()
            end
        end)
    ...
    local function showConfirmDialog()
        ...
        if mn then
            ...
            cb.Text = "−"
            -- Munculkan pill dengan animasi slide dari balik frame
            task.delay(0.25, function()
                syncPositions()
                local finalPos = pill.Position
                local mX = m.AbsolutePosition.X
                pill.Position = UDim2.new(0, mX + 10, finalPos.Y.Scale, finalPos.Y.Offset)
                pill.ZIndex = 1
                pill.Visible = true
                T:Create(pill, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Position = finalPos
                }):Play()
                task.delay(0.35, function() pill.ZIndex = 3 end)
            end)
            task.wait(0.6)
        end
        ...
        local sidebarBlocker = Instance.new("TextButton")
        sidebarBlocker.Size               = UDim2.new(1, 0, 1, 0)
        sidebarBlocker.BackgroundTransparency = 1
        sidebarBlocker.Text               = ""
        sidebarBlocker.ZIndex             = 2001
        sidebarBlocker.Active             = true
        sidebarBlocker.AutoButtonColor    = false
        sidebarBlocker.Parent             = sidebarOverlay

        -- Overlay gelap di pill
        ...
        local pillBlocker = Instance.new("TextButton")
        pillBlocker.Size                  = UDim2.new(1, 0, 1, 0)
        pillBlocker.BackgroundTransparency = 1
        pillBlocker.Text                  = ""
        pillBlocker.ZIndex                = 2001
        pillBlocker.Active                = true
        pillBlocker.AutoButtonColor       = false
        pillBlocker.Parent                = pillOverlay
        ...
        -- Lock sidebar & pill saat dialog terbuka
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        mn = not mn
        if mn then
            -- Minimize: close sidebar dulu kalau open
            if sidebarOpen then toggleSidebar() end
            -- Pill slide masuk ke balik frame lalu hide
            local mX = m.AbsolutePosition.X
            ...
            -- Maximize: sync posisi dulu, lalu pill slide keluar dari balik frame
            task.delay(0.25, function()
                syncPositions()
                local finalPos = pill.Position
                local mX = m.AbsolutePosition.X
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dS and (i.UserInputType == Enum.UserInputType.MouseMovement or
            ...
            -- Gunakan AbsolutePosition m yang sudah benar (bukan newX mentah)
            -- karena m punya Scale component, newX saja tidak mewakili posisi absolut
            task.defer(function()
                if not m or not m.Parent then return end
                local mX  = m.AbsolutePosition.X
                local mY  = m.AbsolutePosition.Y
    ...
    -- Tidak pakai AbsolutePosition signal karena konflik dengan drag handler → flicker
    -- Sync dilakukan: manual di drag, + setelah tween selesai
    pill.Visible = false
    task.wait()
    syncPositions()

    -- Re-sync saat layar di-rotate atau ukuran berubah
    ...
    -- Sync pill setelah animasi masuk selesai, slide keluar dari balik main frame
    task.delay(0.6, function()
        syncPositions()
        local finalPos = pill.Position
        local mRightEdge = m.AbsolutePosition.X + m.AbsoluteSize.X
        pill.Position = UDim2.new(0, mRightEdge - PILL_W, finalPos.Y.Scale, finalPos.Y.Offset)
        pill.ZIndex = 1  -- di belakang main frame saat mulai
        pill.Visible = true
        T:Create(pill, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Position = finalPos
        }):Play()
        task.delay(0.35, function()
            pill.ZIndex = 3  -- balik ke depan setelah animasi selesai
        end)
    end)
