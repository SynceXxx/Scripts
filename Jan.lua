...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    S.Co.Drag = U.InputChanged:Connect(function(i)
        if dragStart and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            ...
            -- Update sidebar langsung di frame yang sama, tanpa delay
            local mAbsX = m.AbsolutePosition.X
            local mAbsY = m.AbsolutePosition.Y
            local mAbsH = m.AbsoluteSize.Y
            sidebar.Position = UDim2.new(0, mAbsX - 48 - 6, 0, mAbsY + 10)
            sidebar.Size     = UDim2.new(0, 48, 0, mAbsH - 20)
        end
    end)
    ...
    local function showHub()
        ...
        m.Visible       = true
        -- Sync posisi sidebar dulu sebelum visible
        syncPositions()
    ...
    m.Position = UDim2.new(0, offX, 0.5, -h/2)
    m.Visible  = true
    -- Sync posisi sidebar berdasarkan posisi awal m, lalu tampilkan
    syncPositions()
    sidebar.Visible = true
    ...
    -- Sidebar ikut otomatis via AbsolutePosition listener di Seidbar.lua
