...
local function showConfirmDialog()
    ...
    -- Coins Input & Button
    Ib("Coins Amount", "CoinsAmount")
    Bt("Add Coins", function()
        if C.CoinsAmount and C.CoinsAmount > 0 then
            Feature.addCoins()
        else
            Feature.showNotification("Please enter valid amount!", false)
            playSound(Sounds.Error, 0.3)
        end
    end, "rbxassetid://112391267091944")
    
    -- XP Input & Button
    Ib("XP Amount", "XPAmount")
    Bt("Add XP", function()
        if C.XPAmount and C.XPAmount > 0 then
            Feature.addXP()
        else
            Feature.showNotification("Please enter valid amount!", false)
            playSound(Sounds.Error, 0.3)
