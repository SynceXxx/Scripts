...
-- Pre-load sounds
local sndOpen    = createSound("85240253037283", 0.5, 1.0)   -- opening (sama kayak close)
local sndClose   = createSound("85240253037283", 0.5, 1.0)   -- closing
local sndReady   = createSound("6026984224", 0.5, 1.0)   -- success saat 100%
...
-- Frame/border custom image (parent ke screenGui biar ga kena ClipsDescendants)
local bgImage = Instance.new("ImageLabel")
bgImage.Size = UDim2.new(0, CONFIG.ContainerWidth + 120, 0, CONFIG.ContainerHeight + 120)
bgImage.Position = UDim2.new(0.5, -(CONFIG.ContainerWidth + 120) / 2, 0.5, -(CONFIG.ContainerHeight + 120) / 2 - 20)
bgImage.BackgroundTransparency = 1
bgImage.Image = "rbxassetid://105018189575284"
bgImage.ScaleType = Enum.ScaleType.Stretch
bgImage.ImageTransparency = 1  -- mulai transparan, fade in barengan loader
bgImage.ZIndex = 3
...
-- Version label (kiri, sejajar percent)
local versionLabel = Instance.new("TextLabel")
versionLabel.Name = "VersionLabel"
versionLabel.Size = UDim2.new(0, 60, 0, 14)
versionLabel.Position = UDim2.new(0, CONFIG.BarPaddingX, 0, 211)
versionLabel.BackgroundTransparency = 1
versionLabel.Text = _G.SynceHubVersion or "[v1.0]"
...
local function fadeInAll()
    ...
    tw(bgImage, {ImageTransparency = 0}, CONFIG.FadeInSpeed):Play()
    ...
    tw(versionLabel, {TextTransparency = 0}):Play()
...
function loader:Remove()
    ...
    tw(versionLabel, {TextTransparency = 1}, slideTime):Play()
    tw(barBackground, {BackgroundTransparency = 1}, slideTime):Play()
    tw(barFill, {BackgroundTransparency = 1}, slideTime):Play()
    -- bgImage slide turun + fade barengan container
    tw(bgImage, {
        Position = UDim2.new(0.5, -(CONFIG.ContainerWidth + 120) / 2, 0.5, -(CONFIG.ContainerHeight + 120) / 2 - 20 + slideDist),
        ImageTransparency = 1
    }, slideTime, Enum.EasingStyle.Quad, Enum.EasingDirection.In):Play()
