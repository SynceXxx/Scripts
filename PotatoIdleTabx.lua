...
local function showConfirmDialog()
    ...
    -- ============================================
    -- SIDEBAR + TOGGLE (di-parent ke ScreenGui, di luar hub)
    -- ============================================
    local sidebarW   = 52
    local sidebarOpen = false

    -- Posisi sidebar relatif terhadap hub (ngikut posisi m)
    -- Sidebar di kiri hub, tinggi sama dengan hub
    local sidebar = Instance.new("Frame")
    sidebar.Name = "Sidebar"
    sidebar.Size = UDim2.new(0, sidebarW, 0, h)
    sidebar.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    sidebar.BorderSizePixel = 0
    sidebar.ZIndex = 5
    sidebar.Visible = false
    sidebar.Parent = g  -- parent ke ScreenGui, bukan m!
    ...
    -- Toggle button "OPEN" — nempel di kiri hub, parent ke ScreenGui
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, 18, 0, 60)
    toggleBtn.BackgroundColor3 = Co.cd
    toggleBtn.Text = ""
    toggleBtn.ZIndex = 10
    toggleBtn.AutoButtonColor = false
    toggleBtn.Parent = g  -- parent ke ScreenGui!
    ...
    -- Fungsi update posisi sidebar & toggle ngikut posisi hub (m)
    local function updateSidebarPosition()
        local mPos  = m.AbsolutePosition
        local mSize = m.AbsoluteSize

        -- Toggle: nempel di kiri hub, tengah vertikal
        toggleBtn.Position = UDim2.new(0, mPos.X - toggleBtn.AbsoluteSize.X - 4, 0, mPos.Y + mSize.Y/2 - 30)

        -- Sidebar: di kiri hub (saat terbuka), sejajar atas hub
        if sidebarOpen then
            sidebar.Position = UDim2.new(0, mPos.X - sidebarW - 4, 0, mPos.Y)
        else
            sidebar.Position = UDim2.new(0, mPos.X, 0, mPos.Y)  -- tersembunyi (sama x dengan hub, nanti geser)
        end
    end

    -- Update posisi sekali di awal
    task.wait(0.05)
    updateSidebarPosition()

    -- Update posisi saat hub di-drag
    S.Co.SidebarSync = game:GetService("RunService").RenderStepped:Connect(function()
        if sidebar.Visible or true then
            local mPos  = m.AbsolutePosition
            local mSize = m.AbsoluteSize
            toggleBtn.Position = UDim2.new(0, mPos.X - 22, 0, mPos.Y + mSize.Y/2 - 30)
            if sidebarOpen then
                sidebar.Position = UDim2.new(0, mPos.X - sidebarW - 4, 0, mPos.Y)
            end
        end
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        ...
        if sidebarOpen then
            -- Posisi awal sidebar: sejajar hub (tersembunyi di balik hub)
            sidebar.Position = UDim2.new(0, mPos.X, 0, mPos.Y)
            sidebar.Visible = true
            -- Slide ke kiri (keluar)
            T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                Position = UDim2.new(0, mPos.X - sidebarW - 4, 0, mPos.Y)
            }):Play()
            T:Create(toggleLabel, TweenInfo.new(0.2), {TextColor3 = Co.ac}):Play()
            toggleLabel.Text = "CLOSE"
        else
            -- Slide ke kanan (masuk balik)
