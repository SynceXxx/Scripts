...
-- Add Coins
function Feature.addCoins()
    if not Remotes.AddCoins then
        Feature.showNotification("AddCoins remote not found!", false)
        return
    end
    
    local amount = C.CoinsAmount
    if not amount or amount <= 0 then
        Feature.showNotification("Please enter a valid amount!", false)
        return
    end
    
    pcall(function()
        Remotes.AddCoins:FireServer(amount)
        Feature.showNotification("Added " .. amount .. " Coins!", true)
    end)
end

-- Add XP
function Feature.addXP()
    if not Remotes.AddXP then
        Feature.showNotification("AddXP remote not found!", false)
        return
    end
    
    local amount = C.XPAmount
    if not amount or amount <= 0 then
        Feature.showNotification("Please enter a valid amount!", false)
        return
    end
    ...
    pcall(function()
        Remotes.AddXP:FireServer(amount)
        Feature.showNotification("Added " .. amount .. " XP!", true)
    end)
end
