...
local function fadeInAll()
    ...
    tw(overlay, {BackgroundTransparency = 0.7}):Play()

    -- Avatar
    task.wait(0.1)
    tw(avatarFrame, {BackgroundTransparency = 0}):Play()
    ...
    -- Sisa text typing setelah nama muncul
    task.wait(0.6)
...
function loader:Remove()
    ...
    tw(container, {
        Position = UDim2.new(0.5, -CONFIG.ContainerWidth / 2, 0.5, -CONFIG.ContainerHeight / 2 + 40),
        BackgroundTransparency = 1
    }, t * 1.5, Enum.EasingStyle.Back, Enum.EasingDirection.In):Play()

    for _, obj in ipairs({containerStroke, avatarStroke}) do
        tw(obj, {Transparency = 1}, t):Play()
    end
    for _, obj in ipairs({avatarImg}) do
        tw(obj, {ImageTransparency = 1}, t):Play()
    end
    tw(avatarFrame, {BackgroundTransparency = 1}, t):Play()
    for _, obj in ipairs({nameLabel, welcomeLabel, statusLabel, percentLabel}) do
        tw(obj, {TextTransparency = 1}, t):Play()
    end
    for _, obj in ipairs({barBackground, barFill, overlay}) do
        tw(obj, {BackgroundTransparency = 1}, t):Play()
    end

    task.delay(t * 1.5 + 0.1, function()
        screenGui:Destroy()
    end)
end
