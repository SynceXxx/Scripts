...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    themeDropdown.Position = UDim2.new(1, -p-108, 0, 48)
    themeDropdown.BackgroundColor3 = Co.cd
    themeDropdown.BorderSizePixel = 0
    themeDropdown.ClipsDescendants = true
    themeDropdown.Visible = false
    themeDropdown.ZIndex = 1000
    themeDropdown.Parent = hd
    
    Instance.new("UICorner", themeDropdown).CornerRadius = UDim.new(0, 10)
    
    local themeStroke = Instance.new("UIStroke", themeDropdown)
    themeStroke.Color = Co.br
    ...
    themeScroll.ScrollBarImageColor3 = Co.ac
    themeScroll.Parent = themeDropdown
    
    local themeList = Instance.new("UIListLayout", themeScroll)
...
local function showConfirmDialog()
    ...
    local function applyTheme(theme)
        ...
        -- Main frame
        if m then
            pcall(function() T:Create(m, tweenInfo, {BackgroundColor3 = colors.bg}):Play() end)
            
            -- Find and update stroke
            local stroke = m:FindFirstChild("UIStroke")
            if stroke then
                pcall(function() T:Create(stroke, tweenInfo, {Color = colors.br}):Play() end)
            end
        end
        
        -- Header frame
        if hd then
            pcall(function() T:Create(hd, tweenInfo, {BackgroundColor3 = colors.bg}):Play() end)
            
            -- Find title and subtitle in header
            for _, child in ipairs(hd:GetChildren()) do
                if child:IsA("TextLabel") then
                    if child.TextSize == 18 then -- Title
                        pcall(function() T:Create(child, tweenInfo, {TextColor3 = colors.tx}):Play() end)
                    elseif child.TextSize == 11 then -- Subtitle
                        pcall(function() T:Create(child, tweenInfo, {TextColor3 = colors.ts}):Play() end)
                    end
                elseif child:IsA("TextButton") then
                    -- Buttons in header
                    pcall(function()
                        T:Create(child, tweenInfo, {BackgroundColor3 = colors.ch, TextColor3 = colors.ts}):Play()
                    end)
                elseif child:IsA("Frame") and child.Name == "ThemeDropdown" then
                    -- Theme dropdown
                    pcall(function()
                        T:Create(child, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                        local dropdownStroke = child:FindFirstChild("UIStroke")
                        if dropdownStroke then
                            T:Create(dropdownStroke, tweenInfo, {Color = colors.br}):Play()
                        end
                    end)
                end
            end
        ...
        -- Find and update divider
        if m then
            for _, child in ipairs(m:GetChildren()) do
                if child:IsA("Frame") and child.Size.Y.Offset == 1 then -- Divider
                    pcall(function() T:Create(child, tweenInfo, {BackgroundColor3 = colors.br}):Play() end)
                    break
                end
            end
        end
        
        -- Update all sections and elements (will be applied to newly created elements)
        Co.bg = colors.bg
        Co.cd = colors.cd
        Co.ch = colors.ch
        Co.br = colors.br
        ...
        -- Update all existing UI elements recursively
        local function updateElement(element)
            ...
            pcall(function()
                if element:IsA("Frame") or element:IsA("ScrollingFrame") then
                    if element.Name:match("Section") or element.Name:match("Card") then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.cd}):Play()
                    end
                    local stroke = element:FindFirstChild("UIStroke")
                    if stroke then
                        T:Create(stroke, tweenInfo, {Color = colors.br}):Play()
                    end
                elseif element:IsA("TextLabel") then
                    if element.Name:match("Title") or element.Name:match("Label") then
                        T:Create(element, tweenInfo, {TextColor3 = colors.tx}):Play()
                    elseif element.Name:match("Desc") or element.Name:match("Subtitle") or element.Name:match("Sub") then
                        T:Create(element, tweenInfo, {TextColor3 = colors.ts}):Play()
                    end
                elseif element:IsA("TextButton") then
                    if not (element.Text == "×" or element.Text == "−" or element.Text == "🎨") then
                        T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch}):Play()
                    end
                elseif element:IsA("TextBox") then
                    T:Create(element, tweenInfo, {BackgroundColor3 = colors.ch, TextColor3 = colors.tx}):Play()
                end
            end)
    ...
    local function populateThemeDropdown()
        if not themesModule or not themesModule.List then return end
        ...
        for i, theme in ipairs(themesModule.List) do
            ...
            themeItem.Size = UDim2.new(1, -4, 0, 28)
            themeItem.BackgroundColor3 = theme.ID == config.CurrentTheme and Co.ac or Co.ch
            themeItem.Text = theme.Name
            themeItem.TextColor3 = Co.tx
            themeItem.Font = Enum.Font.Gotham
            themeItem.TextSize = 11
            themeItem.Parent = themeScroll
            
            Instance.new("UICorner", themeItem).CornerRadius = UDim.new(0, 6)
            
            themeItem.MouseEnter:Connect(function()
                if theme.ID ~= config.CurrentTheme then
                    T:Create(themeItem, TweenInfo.new(0.15), {BackgroundColor3 = Co.cd}):Play()
                end
            end)
            
            themeItem.MouseLeave:Connect(function()
                if theme.ID ~= config.CurrentTheme then
                    T:Create(themeItem, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
                end
            end)
            ...
            themeItem.MouseButton1Click:Connect(function()
                ...
                for _, item in ipairs(themeScroll:GetChildren()) do
                    if item:IsA("TextButton") then
                        T:Create(item, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
                    end
                end
                
                T:Create(themeItem, TweenInfo.new(0.15), {BackgroundColor3 = Co.ac}):Play()
                
                -- Close dropdown
                themeDropdownOpen = false
                T:Create(themeDropdown, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {Size = UDim2.new(0, 180, 0, 0)}):Play()
                task.wait(0.2)
                themeDropdown.Visible = false
            end)
        end
        
        local contentHeight = #themesModule.List * 30
        themeScroll.CanvasSize = UDim2.new(0, 0, 0, contentHeight)
    end
    
    -- Theme button click (toggle dropdown)
    themeBtn.MouseButton1Click:Connect(function()
        ...
        if themeDropdownOpen then
            ...
            local maxHeight = math.min(300, (#themesModule.List * 30) + 10)
            T:Create(themeDropdown, TweenInfo.new(0.25, Enum.EasingStyle.Quint), {
                Size = UDim2.new(0, 180, 0, maxHeight)
            }):Play()
        else
            T:Create(themeDropdown, TweenInfo.new(0.2, Enum.EasingStyle.Quint), {
    ...
    m.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            if themeDropdownOpen and (
                ...
                if not (mousePos.X >= themeBtn.AbsolutePosition.X and 
                        mousePos.X <= themeBtn.AbsolutePosition.X + themeBtn.AbsoluteSize.X and
                        mousePos.Y >= themeBtn.AbsolutePosition.Y and 
                        mousePos.Y <= themeBtn.AbsolutePosition.Y + themeBtn.AbsoluteSize.Y) then
                    closeThemeDropdown()
                end
            end
        end
    ...
    -- ============================================
    -- PRO SOCCER SIMULATOR CONTENT
    -- ============================================
    
    Se("CURRENCY")
    
    -- Toggle: Auto Wins
    Tg("Auto Wins", "AutoWins", function(v)
        Feature.toggleAutoWins(v)
    end)
    
    -- Toggle: Auto Collect Energy (OPTIMIZED!)
    Tg("Auto Collect Energy", "AutoCollectEnergy", function(v)
        Feature.toggleAutoCollectEnergy(v)
    end)
    
    Se("TRAINING")
    
    -- Dropdown: Select Training Mode (G3/G5)
    Dd("Training Mode", Feature.getTrainModes(), "TrainMode")
    
    -- Toggle: Auto Train OP
    Tg("Auto Train OP", "AutoTrainOP", function(v)
        Feature.toggleAutoTrainOP(v)
    end)
    
    Se("SHOP - BALLS")
    
    -- Dropdown: Select Ball Type
    Dd("Ball Type", Feature.getBallTypes(), "BallType")
    
    -- Button: Buy OP Ball (Robux)
    Bt("Buy OP Ball (Robux)", function()
        Feature.buyOPBall()
    end, "rbxassetid://124518238339239")
    
    Se("SHOP - PETS")
    
    -- Button: Buy Pet OP (Hydra x3)
    Bt("Buy Hydra Pet OP (Robux)", function()
        Feature.buyPetOP()
    end, "rbxassetid://86647490838690")
    
    Se("PLAYER")
    
    -- Slider: Walk Speed
    Sl("Walk Speed", 16, 200, "WalkSpeed", function(v)
    ...
    Bt("Rejoin Server", function()
        Feature.rejoinServer()
    end, "rbxassetid://106554243135514")
    ...
    -- Load saved theme on startup (AFTER all UI elements created!)
    if themesModule and config.CurrentTheme then
        task.wait(0.3)
        local savedTheme = themesModule.getTheme(config.CurrentTheme)
        if savedTheme then
