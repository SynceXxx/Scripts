...
local function showConfirmDialog()
    ...
    -- ============================================
    -- SIDEBAR UI (nempel di kiri hub)
    -- ============================================
    local sidebarW  = 48
    local sidebarOpen = false

    -- Sidebar frame (tersembunyi di kiri hub awalnya)
    local sidebar = Instance.new("Frame")
    sidebar.Name = "Sidebar"
    sidebar.Size = UDim2.new(0, sidebarW, 0, h)
    sidebar.AnchorPoint = Vector2.new(1, 0)
    -- posisi default: tersembunyi (di luar kiri hub)
    ...
    -- List layout untuk icon tab di sidebar
    ...
    -- Helper: buat tab icon button di sidebar
    ...
    local function makeSideTab(iconId, panel, order)
        ...
        btn.MouseEnter:Connect(function()
            if selectedTab and selectedTab.deactivate ~= (function() setActive(false) end) then return end
            T:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Co.ach}):Play()
        end)
        btn.MouseLeave:Connect(function()
            -- revert handled by setActive
        end)
    ...
    -- Set home aktif by default
    homeTab.setActive(true)
    selectedTab = homeTab

    -- ============================================
    -- TOGGLE BUTTON (panah buka/tutup sidebar)
    -- nempel di luar kiri hub
    -- ============================================
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, 20, 0, 40)
    toggleBtn.AnchorPoint = Vector2.new(1, 0.5)
    -- posisi: di luar kiri hub, tengah vertikal
    ...
    toggleBtn.MouseButton1Click:Connect(function()
        ...
        if sidebarOpen then
            -- Slide sidebar masuk dari kiri (muncul di dalam hub di sisi kiri)
            sidebar.Position = UDim2.new(0, -sidebarW - 4, 0, 0)
            sidebar.Visible = true
            T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                Position = UDim2.new(0, 0, 0, 0)
            }):Play()
            T:Create(arrowLabel, TweenInfo.new(0.25), {TextColor3 = Co.ac}):Play()
            arrowLabel.Text = "▶"
        else
            -- Slide sidebar keluar ke kiri (tersembunyi)
            T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
                Position = UDim2.new(0, -sidebarW - 4, 0, 0)
            }):Play()
            T:Create(arrowLabel, TweenInfo.new(0.25), {TextColor3 = Co.ts}):Play()
            arrowLabel.Text = "◀"
