...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    local function showHub()
        ...
        -- Posisi final
        local mX = w / 2
        local finalM  = UDim2.new(0.5, -w/2, 0.5, -h/2)
        local sbX     = (workspace.CurrentCamera.ViewportSize.X/2 - w/2) - 48 - 6
        local mY      = workspace.CurrentCamera.ViewportSize.Y/2 - h/2
        local finalSB = UDim2.new(0, sbX, 0, mY + 10)

        -- Mulai dari luar kanan layar
        local offX = workspace.CurrentCamera.ViewportSize.X + 50
        m.Position       = UDim2.new(0, offX, 0.5, -h/2)
        sidebar.Position = UDim2.new(0, offX - 48 - 6, 0, mY + 10)
        m.Visible        = true
        sidebar.Visible  = true

        T:Create(m,       TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = finalM}):Play()
        T:Create(sidebar, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = finalSB}):Play()
    ...
    local function hideHub()
        ...
        T:Create(m,       TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {Position = UDim2.new(0, offX, 0.5, -h/2)}):Play()
        T:Create(sidebar, TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {Position = UDim2.new(0, offX - 48 - 6, sidebar.Position.Y.Scale, sidebar.Position.Y.Offset)}):Play()
    ...
    -- ============================================
    -- ANIMATE IN PERTAMA KALI (main frame + sidebar bareng)
    -- ============================================

    -- Sembunyikan dulu sebelum parent di-set supaya tidak flicker
    m.Visible       = false
    sidebar.Visible = false

    -- Tunggu 2 frame setelah g.Parent di-set biar AbsolutePosition valid
    task.wait()
    task.wait()
    ...
    -- Posisi final m (center layar)
    local finalM = UDim2.new(0.5, -w/2, 0.5, -h/2)

    -- Hitung posisi final sidebar dari AbsolutePosition m setelah settle
    -- Pakai nilai piksel langsung supaya presisi
    local mFinalX = sS2.X/2 - w/2
    local mFinalY = sS2.Y/2 - h/2
    local sbFinalX = mFinalX - 48 - 6
    local sbFinalY = mFinalY + 10
    local sbH0     = h - 20

    -- Set posisi awal (di luar kanan layar) sebelum visible
    m.Position       = UDim2.new(0, offX, 0.5, -h/2)
    sidebar.Position = UDim2.new(0, offX - 48 - 6, 0, sbFinalY)
    sidebar.Size     = UDim2.new(0, 48, 0, sbH0)

    -- Baru tampilkan dan animasi masuk
    m.Visible       = true
    sidebar.Visible = true

    T:Create(m,       TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = finalM}):Play()
    T:Create(sidebar, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0, sbFinalX, 0, sbFinalY)}):Play()

    -- Tunggu animasi selesai lalu syncPositions untuk koreksi posisi presisi
    task.delay(0.55, function() syncPositions() end)
