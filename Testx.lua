...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule)
    ...
    -- Color scheme (default: Neon Purple) — HARUS IDENTIK dengan Themen.lua neon_purple
    local Co = {
    ...
    -- Set CurrentColors dari awal biar notif pertama ngikut tema
    S.CurrentColors = Co
    ...
    -- Gradient Neon Purple langsung dari awal
...
local function showConfirmDialog()
    ...
    cb.MouseButton1Click:Connect(function()
        ...
        -- Hide seluruh hub + sidebar instead of minimize
        m.Visible = false
        sidebar.Visible = false
        sidebarOpen = false
        sidebar.Position = UDim2.new(0, closedX, 0, 0)
        toggleLabel.Text = "OPEN"
        toggleLabel.TextColor3 = Co.ts
    end)
    ...
    local function applyTheme(theme)
        ...
        if theme.ID == "neon_purple" then
            ...
            grad.Color = ColorSequence.new{
                ColorSequenceKeypoint.new(0,   Color3.fromRGB(15, 5, 35)),    -- sangat gelap
                ColorSequenceKeypoint.new(0.5, Color3.fromRGB(40, 10, 80)),   -- ungu gelap
                ColorSequenceKeypoint.new(1,   Color3.fromRGB(80, 25, 140)),  -- ungu medium
            }
            grad.Rotation = 135
            grad.Parent = m
    ...
    -- Update sidebar + toggle colors saat theme berubah
    local function updateSidebarTheme(colors)
        pcall(function()
            local grad = sidebar:FindFirstChildOfClass("UIGradient")
            if grad then
                local r, g2, b = colors.bg.R, colors.bg.G, colors.bg.B
                grad.Color = ColorSequence.new{
                    ColorSequenceKeypoint.new(0, Color3.new(math.min(r*1.2,1), math.min(g2*1.2,1), math.min(b*1.2,1))),
                    ColorSequenceKeypoint.new(1, Color3.new(math.min(r*2,1),   math.min(g2*2,1),   math.min(b*2,1))),
                }
            end
            local sStroke = sidebar:FindFirstChildOfClass("UIStroke")
            if sStroke then sStroke.Color = colors.br end
            toggleBtn.BackgroundColor3 = colors.cd
            toggleLabel.TextColor3 = sidebarOpen and colors.ac or colors.ts
            local tStroke = toggleBtn:FindFirstChildOfClass("UIStroke")
            if tStroke then tStroke.Color = colors.br end
        end)
    end
    ...
    themeBtn.MouseButton1Click:Connect(function()
        ...
        if nextTheme then
            ...
            updateSidebarTheme(nextTheme.Colors)
        end
    end)
    ...
    -- ============================================
    -- TAB PANELS (Home & Settings)
    -- ============================================
    local function makePanel()
        local panel = Instance.new("ScrollingFrame")
        panel.Size = UDim2.new(1, -4, 1, -60)
        panel.Position = UDim2.new(0, 2, 0, 56)
        panel.BackgroundTransparency = 1
        panel.ScrollBarThickness = 2
        panel.ScrollBarImageColor3 = Co.br
        ...
        sP2.PaddingLeft = UDim.new(0, p - 2)
        sP2.PaddingRight = UDim.new(0, p + 2)
        sP2.PaddingTop = UDim.new(0, 2)
        sP2.PaddingBottom = UDim.new(0, p)
        sP2.Parent = panel

        local ly2 = Instance.new("UIListLayout")
        ly2.Padding = UDim.new(0, 6)
        ly2.SortOrder = Enum.SortOrder.LayoutOrder
        ly2.Parent = panel
    ...
    -- sc points to current panel for element creators
    local sc = homePanel
    ...
    -- ============================================
    -- SIDEBAR + TOGGLE (child of m)
    -- Sidebar selalu visible, posisi X yang slide
    -- Tombol di KIRI sidebar (AnchorPoint kanan, nempel di kiri)
    -- Dempet langsung ke hub (0 gap)
    -- ============================================
    local sidebarW = 52
    local toggleW  = 18
    local toggleH  = 60
    ...
    -- closedX: sidebar tersembunyi di kiri hub, hanya toggleBtn yang keliatan
    -- toggleBtn ada di KIRI sidebar, posisinya AnchorPoint(1,0.5) x=0
    -- jadi lebar yang keliatan = toggleW + 2px gap
    local closedX = -(sidebarW - 2)   -- hampir semua sidebar sembunyi, cuma tombol
    local openX   = -(sidebarW)        -- dempet langsung ke hub, 0 gap

    -- Sidebar — child of m, AnchorPoint kanan atas
    ...
    sidebarGrad.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(22, 10, 50)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(35, 18, 70)),
    }
    sidebarGrad.Rotation = 135
    ...
    -- Toggle button — child of sidebar, di KIRI sidebar
    local toggleBtn = Instance.new("TextButton")
    toggleBtn.Name = "SidebarToggle"
    toggleBtn.Size = UDim2.new(0, toggleW, 0, toggleH)
    toggleBtn.AnchorPoint = Vector2.new(1, 0.5)
    toggleBtn.Position = UDim2.new(0, 0, 0.5, 0)  -- kiri sidebar
    toggleBtn.BackgroundColor3 = Co.cd
    toggleBtn.Text = ""
    toggleBtn.ZIndex = 3
    toggleBtn.AutoButtonColor = false
    toggleBtn.Parent = sidebar
    toggleBtn:SetAttribute("ThemeTag", "cd")

    Instance.new("UICorner", toggleBtn).CornerRadius = UDim.new(0, 8)
    local toggleStroke = Instance.new("UIStroke", toggleBtn)
    toggleStroke.Color = Co.br
    ...
    toggleLabel.Rotation = 90
    toggleLabel.ZIndex = 4
    toggleLabel.Parent = toggleBtn
    toggleLabel:SetAttribute("ThemeTag", "ts")

    -- Tab icon builder
    local selectedTab = nil
    local function makeSideTab(iconId, panel, order)
        ...
        btn.Parent = sidebar
        btn:SetAttribute("ThemeTag", "ch")
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 10)

        local btnStroke = Instance.new("UIStroke", btn)
        btnStroke.Color = Co.br
        ...
        ico.Image = iconId
        ico.ImageColor3 = Co.ts
        ico.ZIndex = 3
        ico.Parent = btn
        ico:SetAttribute("ThemeTag", "ts")

        local isActive = false
        local function setActive(active)
            isActive = active
            if active then
                T:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Co.ac}):Play()
                T:Create(btnStroke, TweenInfo.new(0.2), {Transparency = 0.1}):Play()
                T:Create(ico, TweenInfo.new(0.2), {ImageColor3 = Co.tx}):Play()
                btnStroke.Color = Co.ac
                btn:SetAttribute("ThemeTag", "ac")
                ico:SetAttribute("ThemeTag", "tx")
            else
                T:Create(btn, TweenInfo.new(0.2), {BackgroundColor3 = Co.ch}):Play()
                T:Create(btnStroke, TweenInfo.new(0.2), {Transparency = 0.5}):Play()
                T:Create(ico, TweenInfo.new(0.2), {ImageColor3 = Co.ts}):Play()
                btnStroke.Color = Co.br
                btn:SetAttribute("ThemeTag", "ch")
                ico:SetAttribute("ThemeTag", "ts")
            end
        ...
        btn.MouseButton1Click:Connect(function()
            playSound(Sounds.Click, 0.3)
            if selectedTab then selectedTab.deactivate() end
            setActive(true)
            selectedTab = {deactivate = function() setActive(false) end}
            switchTab(panel)
        end)

        btn.MouseEnter:Connect(function()
            if not isActive then
                T:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Co.ach}):Play()
            end
        end)
        btn.MouseLeave:Connect(function()
            if not isActive then
                T:Create(btn, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
            end
        end)

        return {setActive = setActive, deactivate = function() setActive(false) end}
    end

    local homeTab     = makeSideTab("rbxassetid://11036813795", homePanel, 1)
    local settingsTab = makeSideTab("rbxassetid://9307893284", settingsPanel, 2)

    homeTab.setActive(true)
    selectedTab = homeTab

    -- Sidebar background theme tag
    sidebar:SetAttribute("ThemeTag", "cd")

    -- Buka/tutup sidebar
    toggleBtn.MouseButton1Click:Connect(function()
        playSound(Sounds.Click, 0.35)
        sidebarOpen = not sidebarOpen

        if sidebarOpen then
            T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                Position = UDim2.new(0, openX, 0, 0)
            }):Play()
            T:Create(toggleLabel, TweenInfo.new(0.2), {TextColor3 = Co.ac}):Play()
            toggleLabel.Text = "CLOSE"
        else
            T:Create(sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
                Position = UDim2.new(0, closedX, 0, 0)
            }):Play()
            T:Create(toggleLabel, TweenInfo.new(0.2), {TextColor3 = Co.ts}):Play()
            toggleLabel.Text = "OPEN"
        end
    end)

    toggleBtn.MouseEnter:Connect(function()
        T:Create(toggleBtn, TweenInfo.new(0.15), {BackgroundColor3 = Co.ch}):Play()
    end)
    toggleBtn.MouseLeave:Connect(function()
        T:Create(toggleBtn, TweenInfo.new(0.15), {BackgroundColor3 = Co.cd}):Play()
    end)
    ...
    -- Load theme DULU sebelum notifikasi, biar warna notif langsung konsisten
    ...
    -- Baru tampilkan notifikasi, warna sudah pasti pakai tema yang benar
    task.wait(0.2)
