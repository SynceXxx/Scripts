...
-- Feature 4: Auto Wins (KickShoot Loop)
function Feature.toggleAutoWins(enabled)
    if enabled then
        if Connections.AutoWins then return end
        
        Connections.AutoWins = R.Heartbeat:Connect(function()
            pcall(function()
                local Event = RS:FindFirstChild("Events")
                if not Event or not Event:FindFirstChild("Kick") then
                    return
                end
                
                -- Get delay from config (default 0.5s if not set)
                local delay = C.AutoWinsDelay or 0.5
                
                -- Fire Start
                if Event.Kick:FindFirstChild("KickShootStart") then
                    Event.Kick.KickShootStart:FireServer()
                end
                
                task.wait(delay) -- Adjustable delay between start and end
                
                -- Fire End
                if Event.Kick:FindFirstChild("End") then
                    Event.Kick.End:FireServer()
                end
                
                task.wait(delay) -- Adjustable delay before next loop
            end)
        end)
        
        Feature.showNotification("Auto Wins enabled! (Delay: " .. tostring(C.AutoWinsDelay or 0.5) .. "s)", true)
    else
        if Connections.AutoWins then
            Connections.AutoWins:Disconnect()
            Connections.AutoWins = nil
        end
        Feature.showNotification("Auto Wins disabled!", false)
    end
end

-- Feature 5: Auto Collect Energy (OPTIMIZED - No freeze!)
function Feature.toggleAutoCollectEnergy(enabled)
    if enabled then
        ...
        -- Spawn di thread terpisah (NO FREEZE!)
        task.spawn(function()
            -- Cache folder location (cari sekali aja!)
            local collectFolder = nil
            local lastFolderCheck = 0
            
            while running do
                pcall(function()
                    local Event = RS:FindFirstChild("Events")
                    if not Event or not Event:FindFirstChild("Collectable") or not Event.Collectable:FindFirstChild("Collected") then
                        return
                    end
                    
                    -- Re-check folder setiap 10 detik (case respawn/reload)
                    local currentTime = tick()
                    if not collectFolder or (currentTime - lastFolderCheck > 10) then
                        collectFolder = W:FindFirstChild("Collectables") or 
                                      W:FindFirstChild("Energy") or 
                                      W:FindFirstChild("Orbs") or
                                      W:FindFirstChild("Collectable")
                        lastFolderCheck = currentTime
                    end
                    
                    -- Method 1: Scan dari folder yang udah di-cache
                    if collectFolder then
                        for _, item in pairs(collectFolder:GetChildren()) do
                            if running then
                                local id = item:GetAttribute("Id") or 
                                         item:GetAttribute("ID") or 
                                         item:GetAttribute("uuid") or 
                                         item.Name
                                
                                if id and typeof(id) == "string" then
                                    Event.Collectable.Collected:FireServer(id)
                                    task.wait(0.03) -- Small delay antara collect
                                end
                            end
                        end
                    else
                        -- Fallback: Quick scan (cuma Energy/Collectable/Orb objects)
                        for _, obj in pairs(W:GetChildren()) do
                            if running and (obj.Name == "Energy" or obj.Name == "Collectable" or obj.Name == "Orbs") then
                ...
                -- Delay 0.8 detik sebelum scan lagi (Balanced: responsive tapi ga lag!)
                task.wait(0.8)
            end
        end)
        
        Feature.showNotification("Auto Collect Energy enabled! (Optimized)", true)
    else
        if Connections.AutoCollect then
            Connections.AutoCollect:Disconnect()
            Connections.AutoCollect = nil
        end
        Feature.showNotification("Auto Collect Energy disabled!", false)
    end
end

-- Feature 5: Rejoin Server
...
function Feature.resetAllFeatures()
    ...
    -- Turn off Auto Wins if active
    if Connections.AutoWins then
        Connections.AutoWins:Disconnect()
        Connections.AutoWins = nil
    end
    
    -- Turn off Auto Collect Energy if active
    if Connections.AutoCollect then
        Connections.AutoCollect:Disconnect()
        Connections.AutoCollect = nil
    end
