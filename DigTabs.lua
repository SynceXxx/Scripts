...
local function showConfirmDialog()
    ...
    -- Build UI content for Cut Grass
    
    -- ============================================
    -- DIG LEGENDS TAB CONTENT
    -- ============================================
    ...
    homeInfo.Size = UDim2.new(1, 0, 0, 120)
    homeInfo.BackgroundColor3 = Co.cd
    homeInfo.LayoutOrder = nO()
    homeInfo.Parent = sc
    
    Instance.new("UICorner", homeInfo).CornerRadius = UDim.new(0, 12)
    Instance.new("UIStroke", homeInfo).Color = Co.br
    
    local homeTitle = Instance.new("TextLabel")
    homeTitle.Size = UDim2.new(1, -28, 0, 24)
    homeTitle.Position = UDim2.new(0, 14, 0, 12)
    homeTitle.BackgroundTransparency = 1
    homeTitle.Text = "SynceHub - Dig Legends"
    homeTitle.TextColor3 = Co.tx
    homeTitle.Font = Enum.Font.GothamBold
    homeTitle.TextSize = 16
    homeTitle.TextXAlignment = Enum.TextXAlignment.Left
    homeTitle.Parent = homeInfo
    
    local homeDesc = Instance.new("TextLabel")
    homeDesc.Size = UDim2.new(1, -28, 1, -44)
    homeDesc.Position = UDim2.new(0, 14, 0, 40)
    homeDesc.BackgroundTransparency = 1
    homeDesc.Text = "Welcome to SynceHub for Dig Legends!\n\nFeatures:\n• Auto Trophy & Rewards\n• Diamond Exploit\n• Starter Pack Claim\n• Anti-AFK"
    homeDesc.TextColor3 = Co.ts
    ...
    homeDesc.Parent = homeInfo
    
    -- AUTO FARM
    Se("AUTO FARM")
    
    Tg("Auto Trophy", "AutoTrophy", function(val)
        Feature.toggleAutoTrophy(val)
    end, "140507950554297")
    
    Tg("Auto Rewards", "AutoRewards", function(val)
        Feature.toggleAutoRewards(val)
    end, "140507950554297")
    
    -- EXPLOITS
    Se("EXPLOITS")
    
    Bt("Get Diamonds", function()
        Feature.getDiamonds()
    end, "rbxassetid://105330233440321")
    
    Bt("Claim Starter Pack", function()
        Feature.claimStarterPack()
    end, "rbxassetid://105330233440321")
    ...
    Tg("Anti-AFK", "AntiAFK", function(val)
        Feature.toggleAntiAFK(val)
    end, "118025272389341")
    
    -- SETTINGS
    Se("INTERFACE")
    
    Bt("Set Keybind", function()
        if S.WaitingKey then return end
        S.WaitingKey = true
        
        if not kB then return end
        kB.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
        
        local tx = kB:FindFirstChildOfClass("TextLabel")
        if tx then
    ...
    kB = sc:GetChildren()[#sc:GetChildren()]
