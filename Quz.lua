...
function TabContent.createUI(config, state, version, isMobile, featureModule, getKeyName, gameName, themesModule, sidebarModule)
    ...
    local startX = sS2.X + 50
    local mY0    = sS2.Y/2 - h/2
    local sbX0   = sS2.X/2 - w/2 - 48 - 6

    m.Position       = UDim2.new(0, startX, 0.5, -h/2)
    sidebar.Position = UDim2.new(0, startX - 48 - 6, 0, mY0 + 10)
    sidebar.Size     = UDim2.new(0, 48, 0, h - 20)
    m.Visible        = true
    sidebar.Visible  = true

    T:Create(m,       TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.5, -w/2, 0.5, -h/2)}):Play()
    T:Create(sidebar, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0, sbX0, 0, mY0 + 10)}):Play()
